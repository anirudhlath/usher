...
- **Set `USHER_PUSH_ENABLED=false` and `USHER_WORKER_ENABLED=false`, or
  ...
  statable — both return before the gap-closing walks below. Swap
...
PRD 07's four actions all land on `services/watch_write.py`, and their order is
the contract: **write locally** with `origin = api`, so the next sync cannot
mistake Usher's own write for the source's truth; **commit** before anything is
offered to a client ([ADR-0033](../../docs/prd/decisions/0033-an-event-is-a-statement-about-committed-state.md));
**invalidate and publish**, guarded on the row having changed; then **enqueue one
`WATCH_WRITEBACK` job per source copy**, which is deliberately *not* under that
guard — it says nothing about a source left out of step by a parked write-back.
...
- **`_copies` uses two reads deliberately** — `list_for_title` carries
  `AND episode_id IS NULL` and `list_for_episode` is exactly the rows it excludes,
  the unbounded read enqueueing one job per episode *file*. Retracted copies are
  included on purpose, an unmounted drive being no reason to park.

## Emby's own write and read routes

- 🔴 **The write-back route is `POST /Users/{user}/Items/{item}/UserData`** with a
  JSON body (204). The session-scoped `PlayingItems/{item}/Progress` and
  `Sessions/Playing/Progress` answer **400**, keying off a play session Usher
  never has — and `FakeEmbyServer` implemented the adapter's own guess, so forty
  contract assertions passed against a write-back that had never worked.
- **That body must name `Played` even when `Played` is not what is changing** —
  unset fields take DTO defaults, so a position-only body unplays a played item.
- **`DELETE /Users/{user}/PlayedItems/{item}` is destructive beyond its name**: it
  resets `PlayCount`, clears `LastPlayedDate` *and* clears a non-zero resume
  position, so never use it to report an item unplayed — go through `UserData`.
  `POST` to it *is* how to mark played: `PlayCount` to 1 idempotently, a real
  `LastPlayedDate`, and the position cleared, which is what makes PRD 03's
  **"position first, played last"** load-bearing. **A second press is a complete
  no-op**, the object staying byte-identical, so a retry cannot move history on.
- 🔴 **A listing's `UserData` is not an item's.** `GET /Users/{user}/Items`
  ...
  reports the real values, and no `Fields=`/`EnableUserData`/`Ids` spelling fixes
  it. So **`watch_state()`, which walks listings, cannot carry play history**,
  and a walk must never write `play_count`/`last_played_at`
  ([ADR-0014](../../docs/prd/decisions/0014-absence-is-not-zero.md),
  `play_history_is_trustworthy=False`). A pushed `UserDataChanged` *is* honest,
  and `UserData` does not lag, so a read-back needs no polling.
...
`/embywebsocket` upgrades, delivers periodic `Sessions`, and pushes
...
- **`PushSupervisor` resets its failure counter on *delivery*, not on
  ...
  connection means PRD 08's "mark `supports_push = false` after N failures" never
  fires, and catching that needs a fake with an **unbounded** supply of
  connections. `record_reconnect` belongs in `record_open`, guarded on `opened_at`.
...
- **A dropped socket raises `PortUnavailable` rather than hanging, and Emby
  re-delivers nothing**, so **the gap-closing delta is the only cover there is**;
  no real `429` has ever been seen. Keep `configure_logging`, which closes the
  `websockets` token leak, and do not let the queue fill during that walk.
...
watch half for ~11 hours, and **neither log line names it**.

## The match ladder

Tiers in order: `tmdb_id`; `imdb_id`/`tvdb_id`; exact normalised name with year
±1 and exactly one survivor (`_confident`); stub-on-sight.
...
- **Tier 3 is not the fallback its position suggests** — name+year out-resolves
  the `tmdb_id` tier on a real library, most catalog titles carrying no `tmdb_id`.
  A probe with **no** year resolves nothing at all: the year `BETWEEN` propagates
  `NULL`, so "0.0%" there is not a bug.
...
- 🔴 **The availability sweep must not move into a `finally:`, and the obvious
  test shape hides why.** A walk that fails immediately retracts 7 of 7, which
  ADR-0015's ceiling refuses, so the case fails for the wrong reason and never
  exercises a sweep that *succeeds* after a failed walk. The shape with teeth
  commits eight of ten and raises: 20%, under the ceiling, **two available items
  silently retracted**.
...
- ⚠️ **The claim scan walks past backed-off jobs, known and unfixed**, and
  **stub-on-sight is the one path here that is not set-based** — bounded by new
  titles, not items. `.claude/rules/db-and-sql.md` has both plans.

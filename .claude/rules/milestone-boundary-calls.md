...
| **M10** | hardening — OTLP export to a separate Grafana/Prometheus/Loki/Tempo stack, semantic-convention names pinned; a per-source outbound limiter, a ceiling on the gap-closing delta, `KIND_CONCURRENCY` measured, the retraction guard's histogram, drift probe and stated default for a library the household does not own, the leaked `EmbyAdapter` released; `m10c` (the plan's `m10a`) and the suggest writer; the scheduler, off by default, with `search_queries` retention and the neighbour rebuild registered; `usher backup`, `restore` and `rotate-secret` with their runbooks; the suggest metric, the `propose` span, five provisioned dashboards and the alert file; `/health/ready`'s orphaned claims, the problem media type in `/openapi.json`, #10's bounded-column decision, #8 diagnosed; `--version`, `CHANGELOG.md`, the community files, the release workflow and the quickstart | Phase 0: the live catalog, one `GET /search` read back from that stack as a trace, a log line and a metric sample. Phase 1: the live shared Emby, bounded, no walk. Phase 2: every committed panel on real data, and the restore drill into a scratch `pgvector/pgvector:pg17`. J7: the neighbour rebuild on a clone of the live catalog, the scheduler driving only its refusal arm. Phase 3: a clean clone of the public remote against the real Emby — `/home` was never observed with rows |

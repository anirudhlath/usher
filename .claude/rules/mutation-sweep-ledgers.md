---
paths:
  - ".claude/rules/mutation-sweep-ledgers.md"
---

# Mutation sweep ledgers: one entry per task, with its survivors

The per-task record split out of `.claude/rules/mutation-sweeps.md` on
2026-08-21, because that file's `docs/plans/**` trigger fires for almost every
planning task in this repo and 77% of what it loaded was the sweep history of
tasks already shipped. **The reusable half — the harness mechanics, the trap
rules, the scoring vocabulary — stayed there and still loads on that trigger.**

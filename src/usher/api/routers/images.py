...
#: A year, the ceiling RFC 9111 section 1.2.1 puts on a sane `max-age`. The bytes
#: at `(image id, rung)` are what the provider held when they were first fetched
#: and this proxy never re-encodes them, and `uq_images_owner_provider_path` stops
#: the id moving -- which is why `immutable` ships beside it.
...
#: purpose: `source_unavailable` has exactly one status everywhere and no member
#: names a 502, so the transient arm and the non-transient one are told apart by
#: `Retry-After` rather than by code.
...
async def get_image(
    ...
    try:
        ...
        # logo -- so it is an ordinary absence and not an outage.
        raise ProblemException(
...
def _representation_of(request: Request, image_id: uuid.UUID, rung: int) -> str:
    ...
    **Built from the route table and two values this process owns**, never from
    `request.url`: `image_id` is a `uuid.UUID` FastAPI already parsed and `rung`
    came out of `clamp_to_ladder`, so neither can carry a path segment. Reusing
    the request's own path would put a client-supplied byte sequence into a
    response header on an internet-facing service.

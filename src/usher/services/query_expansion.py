...
#: Spelled once, so the prompt, the schema and the reader cannot drift -- a
#: schema saying `query` beside a reader saying `expanded` drops 100% of a
#: correct answer and bills for it.
QUERY_KEY = "query"
...
class QueryExpansionService:
    ...
    `client` is `LLMClient`, never `LLMClient | None`. `composition.llm_client`
    answers `(None, no-op)` for `USHER_LLM_ENABLED=false`, and the root also
    declines to build this service whenever `USHER_QUERY_EXPANSION_ENABLED` is
    `false` -- the default even where a client exists. The optionality a *search*
    genuinely needs lives one layer up, on `SearchService.expander`, because a
    `SearchService` is built on every deployment and this is not.

    `model` is `settings.llm_model` and is not defaulted. It is the same string
    `OpenAICompatibleClient` was built with, and the only honest value for
    `llm_calls.model` on the path where no response came back to read one from.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._client = client
        # The ledger rule is `services/llm_ledger.py`'s, not this module's. A
        # verbatim copy of `CurationService`'s settle/record pair here would be a
        # second spelling, and a commit dropped from one of two spellings is
        # invisible.
        ...
        # window. One clock, two readings -- a second callable would make
        # `elapsed_ms` a delta between two different clocks.
        self._clock = clock
    ...
    async def expand(self, query: str) -> str | None:
        ...
        try:
            ...
            # `UsherPortError` and never `Exception`: `_record`'s rule, one method
            # up.
            expanded, error = None, str(exc) or type(exc).__name__
        else:
            expanded = read_expansion(payload)
            # The call worked, the money is spent, and the attempt produced
            # nothing. `ok = false` with real tokens and a real cost is the only
            # thing separating that from a call that never reached the endpoint.
        ...
        if error is not None:
            ...
            # absorbed, so the viewer gets results and `_print_search_answer` prints
            # no `expanded:` line -- an absence, which says nothing on its own.
            logger.warning(
    ...
    async def _settle(self, started: float, *, usage: LLMUsage | None, error: str | None) -> None:
        ...
        `generation_id` stays `None` and is the ledger's default rather than this
        method's argument. This purpose produces no `curated_rows` at all, so an
        id minted here would be a join key pointing at nothing, and PRD 10's
...
def _schema() -> dict[str, Any]:
    ...
    An optimisation, never the contract, which is why `read_expansion` checks
    the same three things whatever the provider did. `additionalProperties:
    false` plus a `required` naming every property is what `strict: true`
    demands, and the key is `QUERY_KEY` rather than a literal so the schema and
    the reader are one definition.

    No `maxLength`, the same call curation makes about `minItems`. A length
    keyword under guided decoding does not make a model answer shorter -- it
    stops the decoder mid-sentence at exactly the bound, arriving as valid JSON
    with no error anywhere. The bound is a `description` hint here and a refusal
    in `read_expansion`, where an over-long rewrite is discarded whole.

...
# `ScoredRow.pinned`, not this number. PRD 06's "1 row, always ranked first" is
# positional, and a guarantee expressed as "a score high enough to win" is one
# another provider's arithmetic can silently take away on a screen that still
# looks fine.
CONTINUE_WATCHING_SCORE = 1.0

_SLUG = "continue-watching"

# PRD 06's `SourceRow` figure, unmodified. The one row that must not survive the
# user pressing stop, which is why it is the shortest TTL in the set.
...
class Resume:
    """One shelf entry, with a chapter when the household's place is an episode."""
...
class ContinueWatchingRow(BaseRow):
    def __init__(self, entries: Sequence[Resume]) -> None:
        ...
        # `build`. Two reads at two instants can disagree, and a *stricter* `build`
        # would silently suppress a row that was fine when proposed.
    ...
    def reason(self) -> str | None:
        # Written to be spoken aloud rather than merely displayed -- PRD 06's
        # Alfred section states that as a constraint on the field.
    ...
    def display_hint(self) -> DisplayHint:
        # The resume hint. This is the only family where the card's progress is
        # the point, and a poster hint loses the bar.
...
class ContinueWatchingProvider(RowProvider):
    ...
    async def propose(self, ctx: RowContext) -> Sequence[ScoredRow]:
        ...
        if not states:
            # Nothing, not an empty row and not a generic one. An empty sequence
            # is a first-class answer here: the household has started nothing, or
            # has finished everything it started, and both are ordinary.
            return []

        # The episode roll-up is here rather than in the repository, and
        # `list_in_progress`' own docstring hands it over: *"Collapsing to one card
        # per series is the provider's, and is decided once, there."* An episode's
        # watch state carries a NULL `title_id`, so a provider that skipped this
        # drops every episode resume.
        ...
        for state in states:
            ...
            # One card per series. Ten episodes of one show in progress is one
            # shelf entry, and the most recent wins because `list_in_progress`
            # already ordered them by recency.
        ...
        return [
            ScoredRow(
                ...
                # PRD 06's "always ranked first", as a flag rather than a score.
                pinned=True,
            )
        ]

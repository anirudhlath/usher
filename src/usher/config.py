...
from usher.domain.jobs import BOOTSTRAP_CONCURRENCY
...
class Settings(BaseSettings):
    ...
    def _the_pool_can_hold_the_worker(self) -> "Settings":
        ...
        two more, from the same pool, for the whole import: the connection its
        dataset's hold lives on, and the one the datasets its phase reads are
        held shared on. Over the pool's capacity, jobs do
        ...
        shape.
        """
        imports = min(self.job_concurrency, BOOTSTRAP_CONCURRENCY)
        needed = self.job_concurrency + 2 + 2 * imports
        capacity = self.db_pool_size + self.db_max_overflow
        if needed > capacity:
            raise ValueError(
                f"USHER_JOB_CONCURRENCY={self.job_concurrency} needs {needed} connections "
                f"(one per job in flight, plus the claim, the heartbeat, and the hold and the "
                f"reads of each of up to {imports} bootstrap imports) and "

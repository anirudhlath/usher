...
paragraph. ⚠️ **`m10c` created the two indexes verbatim and this paragraph is
the M8 reading.** Their reader is Grafana rather than the port, which is still
append-only; the measurement asked for here is the one in
`tests/integration/test_cost_anomaly_query.py`. **Not indexed even then:
`purpose` and `model`** — a deployment holds one or two values of each, so a
btree over either is a structure with two entries, which is
`title_embeddings.model_name`'s refusal one module over.

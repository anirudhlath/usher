...
#: Per table, because the answer differs per table and is not a function of anything
#: else the artifact records: one field to read, rather than the argument re-derived
#: at each call site.
...
class Unresolved:
    ...
    **Not `None`.** `None` is a value a caller can write, and one written into
    `watch_states.title_id` surfaces as a foreign-key error out of Postgres
    instead of *"this watch state's title is not in the target"*. A distinct
    type makes the two impossible to confuse, and gives the restore report
    something to name and count.
    """
...
def title_reference(title: Title) -> TitleReference:
    ...
    Every key the row has, not the strongest one: the target may be a catalog
    at a different bootstrap phase, so a title carrying an `imdb_id` here can
    be a row carrying only a `tmdb_id` there, and a partially-crosswalked
    target is the ordinary state of a fresh install. Dropping rungs at *write*
    time would decide the resolution before the target is known.
    """
...
def episode_reference(episode: Episode, series: TitleReference) -> EpisodeReference:
    ...
    Takes the series' reference rather than its `Title`: the caller building a
    page of episodes already holds one per series, and rebuilding it per
    episode costs a `title_reference` call per row on a library that is mostly
    episodes.
    """
...
async def resolve_titles(
    ...
    **Every** reference, including one that repeats -- two watch states for
    one title is the ordinary shape of a household -- because restore iterates
...
async def _answer[ReferenceT: (TitleReference, EpisodeReference)](
    ...
    """The one place an absence becomes a refusal, shared so both arms agree on it.

    An empty batch asks nothing: `resolve_natural_keys([])` is a round trip to
    learn nothing, the same guard `list_by_ids` and `resolve_tmdb_ids` carry
    one port up.

    `references` is passed whole because the port already deduplicates its own
    probe list -- a household naming one title from every watch state costs one
    probe here and none extra.

    Membership rather than falsiness: the resolver omits what it could not
    find, and a `uuid.UUID` being truthy is a fact about a type this module
    does not own.
    """
    if not references:
        return {}

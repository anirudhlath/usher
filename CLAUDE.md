...
| **M10 Phase 1** (Safety, Group S — S1–S11, in progress on `milestone/m10-hardening`) | the outbound rate gate (`_MinInterval`, `SourceGateRegistry`, ADR-0040), the 429 chain end to end, a cursorless delta refused, `USHER_PUSH_GAP_MAX_ITEMS`, the four `KIND_CONCURRENCY` entries turned into measurements, `usher.sync.retraction.fraction`, a failed sync run exiting non-zero, and a leaked push adapter released | a real Emby 4.9.5.0 — **S1 52 requests, S7 142, S8 1, S11 11** |
...
observed"* because provoking it is the behaviour ADR-0040 exists to prevent;

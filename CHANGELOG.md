...
- **One corrupt Emby item no longer aborts a whole sync.** An integer too large
  for its column, such as a runtime of 111 years, used to crash the walk and
  leave its run `running`. It is now stored as unknown.

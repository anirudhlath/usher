...
One socket held 100 minutes against the same 4.9.5.0 build, driving the
shipped `EmbyPushChannel` over the real `websockets` client: **200 frames —
183 `Sessions`, 5 `UserDataChanged`** (three of them from one restorative
...
|---|---|---|
| The envelope carries `MessageId` at all | **half confirmed, half refuted** | `UserDataChanged` and `LibraryChanged` carry one; **`Sessions` carries none**, on 183 of 183 frames. `push_sessions.json` no longer has one. |

...
class FakeCreditRepository(CreditRepository):
    def __init__(
        ...
        # Wired to the *same* `titles` store the caller reads through, for the reason
        # `FakeTitleRepository`/`FakeTitleMatchRepository` are wired together:
        # `credit_names` is one column, and two independent dicts would make a *correct*
        # implementation fail rather than a wrong one pass.
        self._titles = titles or FakeTitleRepository()
        self._credits: list[Credit] = []
        # `title_search_names`, keyed by the delete's own scope.
    ...
    async def replace_for_titles(
        ...
        *,
        ...
        # ...and `title_search_names`' `person` half in the same call, from the same
        # mapping, for the same scope.
        for title_id in scope:

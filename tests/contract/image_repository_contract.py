...
class ImageRepositoryContract:
    ...
    async def test_primary_for_titles_falls_back_to_the_first_when_nothing_is_flagged(
        ...
        """A provider that flags nothing is ordinary.

        TMDb publishes no "primary" bit at all, so `is_primary` is a judgement the
        derivation makes, and a derivation that declines to make it must not cost the
        title its artwork.
    ...
    async def test_get_answers_the_stored_image_by_its_own_id(
        ...
        """The serve path's resolve.

        `GET /images/{id}` holds an id and nothing else, and needs the provider and the
        path to build a fetch URL.

        A second title's image is stored alongside, so a `get` that returns whatever it
        found first fails here.
        """
        title_id = await seeder.title()
        other = await seeder.title()
        await repository.replace_for_titles(
    ...
    async def test_replace_reports_the_number_of_rows_written(
        ...
        `usher derive`'s report is the one thing that tells an operator a derivation
        reached this table at all.

        Counted after deduplication, so it is what was written and not what was handed
        in.
        """
        title_id = await seeder.title()
        written = await repository.replace_for_titles(
    ...
    async def test_a_scope_with_no_rows_still_empties_its_titles(
        ...
        `title_ids` non-empty with `images` empty is exactly that state, and it is the
        ordinary shape of a provider withdrawing a film's only poster.
        """
        title_id = await seeder.title()

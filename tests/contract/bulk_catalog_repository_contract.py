...
def _genome(movie_id: int, imdb_id: str, lead: float) -> GenomeVector:
    """One full-width vector whose first lane is `lead` and whose rest is zero.

    Full width because `halfvec(1128)` rejects anything else, and only the first lane is
    ever asserted on -- the cases here are about *which key* the vector lands under, not
    about its contents.
    """
    return GenomeVector(
        movie_id=movie_id,
...
class BulkCatalogRepositoryContract:
    async def test_upsert_titles_inserts_then_reports_no_change_on_replay(
        ...
        If a replayed batch reported inserts, a crash-and-resume would duplicate rows;
        if it reported updates, every no-op re-import would fire the set_updated_at
        trigger across the catalog.
        """
    ...
    async def test_upsert_titles_never_discards_enrichment_set_by_a_different_pass(
        ...
        """The headline safety property this port states and nothing exercised.

        "a re-import ... must never downgrade an enriched title", from the
        method's own port docstring. An implementation that replaces the whole
        stored row on update, rather than touching only the IMDb-supplied
        columns, would discard tmdb_id/popularity a *later* crosswalk or TMDb
        pass wrote; this only pins the two fields this port itself can read
        back.
        """
        await repo.upsert_titles([SHAWSHANK])
    ...
    async def test_upsert_crosswalk_never_blanks_a_column_another_pass_filled(
        ...
        """The three SPARQL joins run as three separate passes, each carrying one column.

        Without COALESCE on the stored side, the P4983 pass would wipe every
        tmdb_movie_id the P4947 pass wrote.
        """
        await repo.upsert_crosswalk([IdCrosswalkPair(imdb_id="tt99000020", tmdb_movie_id=90000020)])
    ...
    async def test_link_crosswalk_never_overwrites_an_existing_tmdb_id(
        ...
        """The NULL precondition is on the target row, not on the incoming pair.

        "Only fills a tmdb_id ... that is currently NULL" (this method's own
        port docstring), not just "does the incoming pair agree with what's
    ...
    async def test_link_crosswalk_enforces_a_global_unique_tvdb_id(
        ...
        Wikidata can associate one tvdb id with more than one imdb_id; the real
        implementation resolves that by keeping the lexicographically smallest imdb_id
        (`DISTINCT ON (tvdb_series_id) ... ORDER BY tvdb_series_id, imdb_id`), and the
    ...
    async def credit_names_of(
        ...
        """How this implementation reads back `titles.credit_names`.

        Same not-on-the-port reasoning as `popularity_of`: nothing in production reads
        it through *this* port -- `TitleRepository.credit_names_for` is what
        `services/index.py:116` calls.
        """
        raise NotImplementedError
    ...
    async def title_id_of(self, repo: BulkCatalogRepository, imdb_id: str) -> uuid.UUID | None:
        """The `titles.id` an IMDb id resolved to.

        A test affordance, not a port method: `BulkCatalogRepository` deliberately
        exposes no per-row read, and the whole point of the case above is that the
        *stored key* is this id.
        """
        raise NotImplementedError
    ...
    async def genome_keys(self, repo: BulkCatalogRepository) -> set[object]:
        """Every key `genome_scores` is stored under.

        Read as a set of opaque objects so the case that a `movieId` is *not* among them
        can be written without either arm having to pretend an integer is a UUID.
        """
        raise NotImplementedError

    async def enrich(self, repo: BulkCatalogRepository, imdb_id: str) -> None:
        """Move a title off the skeleton tier.

        Nothing on this port writes `enrichment_state`, and the enriched-tier coverage
        fraction is the one number that matters, so the suite needs a way to create one.
        """
        raise NotImplementedError

    async def popularity_of(self, repo: BulkCatalogRepository, imdb_id: str) -> float | None:
        """How this implementation reads back a title's popularity.

        Not on the port: nothing in production needs it, and adding a read method to
        satisfy a test would widen the port for no caller.
        """
        raise NotImplementedError

    async def tmdb_id_of(self, repo: BulkCatalogRepository, imdb_id: str) -> int | None:
        """How this implementation reads back a title's tmdb_id.
    ...
    async def tvdb_id_of(self, repo: BulkCatalogRepository, imdb_id: str) -> int | None:
        """How this implementation reads back a title's tvdb_id.
    ...
    async def indexes_intact(self, repo: BulkCatalogRepository) -> bool:
        """Whether whatever `bulk_load_window` suspended is back.

        Trivially True for a fake that suspends nothing.
        """
        raise NotImplementedError

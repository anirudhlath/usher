...
async def test_the_route_walk_finds_the_shipped_surface(app: FastAPI) -> None:
    """The premise for the walk below.

    An empty walk, or one that found only FastAPI's `/docs`, would make every assertion
    in the sweep vacuous -- and that is not hypothetical here, because a one-level walk
    over `create_app().routes` finds exactly that and nothing else.
    """
...
async def test_an_unrouted_path_is_a_problem_document_too(client: httpx.AsyncClient) -> None:
    """Starlette's own 404, raised by the router before any of Usher's code runs.

    A handler registered only for `fastapi.HTTPException` would miss it, and a client
    would meet two different 404 shapes.
    """
...
async def test_a_malformed_titles_filter_is_a_problem_document(
    ...
    """`GET /events`'s only failure, hand-raised at `routers/events.py`.

    The route is on the exemption list for its *stream* -- once it has answered `200
    text/event-stream` there is no status code left to carry a document -- and that
    exemption must not swallow the 422 it answers instead.
    """
...
async def test_a_rejected_registration_carries_the_stripped_pydantic_errors(
    ...
    `loc`/`msg`/`type`/`ctx` survive as an RFC 9457 **extension member**, so a client
    still learns which field was wrong and why.
    """
...
async def test_the_document_status_agrees_with_the_status_line_on_every_failure(
    ...
    The structural half is the case below -- these two are not the same claim, because
    two values that happen to agree today are what a document built beside its response
    also produces.
    """
...
def test_the_response_status_is_read_from_the_document_rather_than_passed_beside_it() -> None:
    """`status` is written once, and this is the assertion that keeps it that way.

    Behaviour cannot make the claim: a handler that passed the same integer to the
    document and to the response answers identically today and diverges the first time
...
async def test_health_and_readiness_keep_their_own_shapes(client: httpx.AsyncClient) -> None:
    ...
    `/health/ready`'s real consumers -- Kubernetes, Docker `healthcheck`, load balancers
    -- gate on the code and never parse the body, so its 503 stays `ReadinessResponse`.
    The DSN this app carries is unreachable, which is what makes the 503 arm reachable
    in a unit test.
    """
...
async def test_events_still_answers_a_stream(app: FastAPI) -> None:
    ...
    Asserted through the streaming transport because `httpx.ASGITransport` buffers a
    response to completion and would hang on this route rather than fail it.
    """
    async with LifespanManager(app) as manager:

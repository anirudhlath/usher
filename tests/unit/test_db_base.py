...
def test_base_declares_metadata_with_the_naming_convention() -> None:
    """`hasattr(Base, "metadata")` alone is a tautology.

    every DeclarativeBase subclass gets one from the framework, whether or not
    usher.db.base does anything with it.

    What Base actually needs to prove is that the naming convention (constraint names
    under our control, not Postgres-generated ones -- see NAMING_CONVENTION's docstring)
    is wired up, since that's the one piece of behaviour this module adds.
    """
...
def test_session_factory_does_not_expire_on_commit() -> None:
    ...
    `hasattr(session, "execute")` (true of nearly any session-shaped object) would not

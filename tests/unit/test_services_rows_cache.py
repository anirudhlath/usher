...
def meter_reader() -> Iterator[InMemoryMetricReader]:
    """`usher.cache.hits`/`.misses` are exercised in depth in `test_telemetry_cache.py`.

    This fixture is here only for the one boundary case below, so the expiry habit
    `stale_after` teaches is checked against the metric in the same place it is
    checked against the returned value.
    """
...
class _Clock:
    ...
    `datetime.now(UTC)` inside the cache makes both TTL cases unassertable: there is
    no way to reach an expiry without sleeping, and a case that sleeps 30 s is a case
    nobody runs.
    """
...
def test_two_users_never_share_a_composed_screen(clock: _Clock) -> None:
    """The key is built from the `user_id` the request resolved, not from a constant.

    v1 mints one user, so a key that omitted `user_id` would work today and serve one
    household's screen to another the day the authentication seam is replaced -- with
    no error, no log line and no metric.
...
def test_an_entry_exactly_at_its_expiry_is_expired(
    ...
    """Steps the clock *onto* the TTL boundary, not past it.

    A case that steps past the boundary agrees with both `<=` and `<`, so the
    one-character difference between them never shows; one second of drift in a 30 s
    TTL is invisible.

    **And an entry found expired is a `usher.cache.misses` point, not a
    `usher.cache.hits` one** -- it is a rebuild, not a serve. The wrong implementation
    this rules out records the miss on `<=` correctly but the *metric* on whether
    `entry` was found at all, which an expired-but-present entry would count as a hit.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    cache.put_screen(user, _screen("old"), ttl=_TTL)
...
def test_a_stale_serve_is_a_hit_labelled_stale(
    ...
    """A served-stale read is a hit, and a hit carrying `freshness`.

    A **hit**, because the request was served out of the cache and paid no rebuild:
    counting it a miss would make `usher.cache.hits` say a compose happened when none
    did. But *not a plain hit*, because a plain hit hides exactly what serve-stale
    trades away -- the household is looking at data older than the TTL.

    `freshness` is on the hits counter only. A miss served nothing, so it has no
    freshness to report, and the pair stays at four series. The wrong implementations
    this rules out: a stale serve counted as a miss, and a stale serve counted under
    the same series as a fresh one.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    cache.put_screen(user, _screen("old"), ttl=_TTL)
...
def test_an_entry_exactly_at_the_end_of_its_grace_is_a_hard_miss(clock: _Clock) -> None:
    """Steps exactly onto the second boundary, not past it.

    `TTL + grace` is the instant a stale entry stops being servable, and `>=` against
    `>` there is one keystroke, invisible to every case that steps past. Past it the
    entry is *removed*, not merely refused -- the screen half is bounded only by the
    `users` table, so an entry nobody will ever be served again is dead weight per
    household.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    grace = dt.timedelta(seconds=60)
...
def test_the_grace_is_read_at_the_read_and_not_baked_in_at_the_write(
    ...
    """A grace folded into the stored `expires_at` would be a different feature.

    It reads identically at every assertion above: the entry would then be *fresh* for
    `TTL + grace`, so nothing would ever be stale, no refresh would ever be scheduled,
    and the screen a household sees would simply live 90 s instead of 30. What
    distinguishes the two is a reader with **no** grace looking at the same entry a
    second before the TTL expires and a second after.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    cache.put_screen(user, _screen("old"), ttl=_TTL)
...
def test_the_row_cache_is_bounded_because_its_key_space_is_the_catalog(
    ...
    """`because-you-watched-<seed>` is one slug per seed, so an unevicted dict leaks.

    Keyed by `(user_id, slug)` it grows with the household's watch history, and
    expired entries are only *read* past, never removed, so the TTL reclaims nothing.
    """
...
def test_eviction_takes_the_soonest_to_expire_first(clock: _Clock) -> None:
    """A ceiling that evicted the *newest* entry never serves what it was just given.

    Bounded, and useless, with no symptom but a miss rate.
    """
...
def test_invalidating_a_row_also_drops_the_screen_that_contained_it(clock: _Clock) -> None:
    """The screen is a *composition of rows*, so a moved row leaves the screen stale.
...
def test_enriching_a_title_drops_every_cached_row_that_names_it(clock: _Clock) -> None:
    """A row is built from titles, so a changed title leaves every row holding it stale.

    The card carries `name`, `year`, `enrichment_state` and `artwork`, and enrichment
    rewrites all four. The household that never cached the title keeps both halves,
    which is what makes this a statement about the *title* rather than a `clear()`
...
def test_a_screen_naming_the_title_goes_even_when_its_row_was_never_cached(
    ...
    half would leave the next request a screen cache hit carrying the stale card --
    the same subtle failure arriving through the other door.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    cache.put_screen(user, _screen("enriched"), ttl=_TTL)
...
def test_invalidating_no_titles_drops_nothing(clock: _Clock) -> None:
    """An empty batch clears nothing, which keeps the three cases above about titles.

    An empty batch is the shape a job with nothing enriched hands over, and a
    `clear()` behind this name would satisfy every assertion of theirs while emptying
    a cache no write had staled.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
    cache.put_row(user, "because-you-watched", _row("enriched"), ttl=_TTL)
    cache.put_screen(user, _screen("enriched"), ttl=_TTL)
...
def test_clearing_empties_both_halves(clock: _Clock) -> None:
    """`usher home --repeat` clears between runs, so every run is a cold compose.

    A repeat served out of the cache would report a number near zero and mean nothing.
    """
    cache, user = RowCache(clock=clock), uuid.uuid4()
...
async def test_a_second_compose_inside_the_window_rebuilds_nothing(
    ...
    """The screen cache is asserted through the composer: a cache nothing reads is a dict."""
    cache = RowCache(clock=clock)
...
async def test_a_compose_after_the_screen_expires_builds_again(
    ...
    """The half that fails against a TTL that never expires.
...
async def test_a_row_survives_the_screen_expiring_because_its_own_ttl_is_longer(
    ...
    """Two cache layers, because a screen lives ~30 s and a similarity row lives hours.

    A composer that only cached the screen would rebuild every row on every screen
    miss, doing the expensive half of the work on a 30 s cycle for rows whose inputs
    move in days.
    """
    cache = RowCache(clock=clock)

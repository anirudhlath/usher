...
def test_a_season_number_may_be_zero_for_specials() -> None:
    """TMDb numbers a series' specials as season 0, and Emby emits `ParentIndexNumber.

    0` for them.

    Rejecting zero would drop every special in the library.
    """
...
def test_a_negative_season_number_is_rejected_on_an_episode_too() -> None:
    """`Season.season_number` and `Episode.season_number` are two separate bounds on two.

    separate models, and only the first had a case.

    Deleting this one's `ge=0` left the suite green -- verified by running exactly that
...
def test_a_negative_absolute_number_is_rejected() -> None:
    """Not in the plan's original case list.

    which nonetheless told the implementer to mutation-test `absolute_number`'s `ge=0`.

    An unenforced constraint is worse than no constraint, because it reads as enforced.
    """
    with pytest.raises(ValidationError):
        Episode(
            title_id=TITLE_ID,
...
def test_a_malformed_imdb_id_is_rejected() -> None:
    r"""Same `^tt\d{7,8}$` pattern `Title.imdb_id` carries.

    An episode's IMDb id is a matcher input exactly as a title's is, and a
    source that emitted `"2178782"` without the prefix would otherwise be
...
def test_evolve_revalidates_where_model_copy_would_not() -> None:
    """`.evolve()` is the only sanctioned write path precisely because it re-runs validation.

    `model_copy(update=...)` would hand back an `Episode` with a negative episode number
    that serializes without complaint (CLAUDE.md, `DomainModel`).
    """
    episode = Episode(title_id=TITLE_ID, season_id=SEASON_ID, season_number=1, episode_number=1)
    with pytest.raises(ValidationError):
...
def test_both_models_reject_an_unknown_field() -> None:
    """`extra="forbid"` from `DomainModel`.

    A typo'd keyword in the TMDb mapper (`episode_num=`) must fail at construction, not
    be dropped.
    """
    with pytest.raises(ValidationError):
        Season(

"""Enum-level behavior that individual model tests shouldn't have to re-derive."""
...
def test_enrichment_state_str_ordering_is_not_the_ladder() -> None:
    ...
    A "don't downgrade" guard written as `new_state > old_state` on the enum members
    themselves type-checks, runs, and silently computes the wrong answer.

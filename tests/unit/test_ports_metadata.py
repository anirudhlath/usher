...
def test_enrichment_returns_the_aggregate_not_just_a_title() -> None:
    """The 🔶: `to_title` returned a `Title` while the stage populates a hierarchy.

    `seasons`/`episodes` are here because M4 stores them;
    `people`/`credits`/`images`/`collection` are deliberately absent because nothing in
...
def test_the_verbatim_payload_travels_with_the_result() -> None:
    ...
    `raw_payloads`, so M7 and M9 re-derive them with no second network call.

    PRD 02's whole stated purpose for that table.
...
def test_an_enrichment_result_carries_the_hierarchy_a_series_needs() -> None:
    """999,827 of the measured source's 1,126,674 items are episodes.

    so a result that could not carry seasons and episodes would leave the pipeline
...
def test_an_enrichment_result_is_frozen() -> None:
    """A mutable aggregate handed to `EnrichService` and then written to `raw_payloads` could be.

    edited between the two, so the cached payload would stop being what the provider
    actually answered.
    """
...
def test_fetch_takes_a_provider_ref_not_a_bare_integer() -> None:
    ...
    `provider_id: int` baked in TMDb's id scheme, which IMDb's `tt99000100` does not fit
    -- and PRD 01 lists additional metadata providers as an open extension seam.
...
def test_fetch_no_longer_takes_a_separate_kind() -> None:
    ...
    `None` for a global namespace like IMDb's.

    A second `kind` argument would make "a TMDb ref with no kind" and "an IMDb ref with
    a kind" both spellable, and ADR-0011 is what happens when the first one is.
    """
...
def test_to_result_replaced_to_title() -> None:
    ...
    silently does not is the 🔶 restated as an API.
    """
...
def test_changed_since_is_resumable() -> None:
    ...
    `days: int` in, `list[int]` out cannot express a cursor through TMDb's paginated,
    14-day-capped changes feed, so a partial run had no way to pick up where it stopped.
    """
...
def test_a_changed_page_carries_refs_not_bare_integers() -> None:
    ...
    a page of ids the caller then has to pair with a kind is ADR-0011's failure waiting
    for a caller to make it.
...
def test_search_can_be_scoped_to_one_kind() -> None:
    """TMDb keys movies and series in separate spaces and searches them through separate.
...
def test_a_search_candidate_still_speaks_the_canonical_vocabulary() -> None:
    """`MetadataCandidate` is unchanged and deliberately so (ADR-0017).

    its `provider_id` plus `kind` plus the provider's own `name` is losslessly a
    `ProviderRef`, and the M1 bug it replaced -- `search()` returning `list[dict[str,
    Any]]`, which made the match stage index into TMDb's movie/TV divergence -- stays
    fixed either way.
    """
...
def test_a_derivation_carries_the_fourth_entity_and_a_provider_cannot_forget_it() -> None:
    """ADR-0016 kept `raw_payloads` so four entities could be re-derived from it.

    M7 cashed three and `images` is the fourth.
...
def test_to_derivation_is_synchronous_and_pure_for_all_four_entities() -> None:
    """The clause the whole stage rests on.

    and `images` is the field where a reader would most expect it to be broken --
...
def test_a_complete_metadata_provider_implementation_instantiates() -> None:
    """The ABC-shape check ADR-0001 exists for.

    an implementation missing a method fails at construction, not at the call site five
    layers into a walk.
    """
...
def test_a_provider_that_still_implements_the_old_shape_is_incomplete() -> None:
    """The settling is enforced by the ABC rather than by review.

    a provider written against the pre-M4 signatures no longer satisfies the port.
    """

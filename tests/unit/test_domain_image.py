...
def test_image_and_image_row_have_matching_field_sets() -> None:
    ...
    checked rather than assumed.** `title.py`, `episode.py` and `people.py` all carry it
    and `tests/unit/test_db_models.py`'s assertion is scoped to `TitleRow`/`Title`, so
    `m09a`'s note that "four rows with no domain twin break nothing" was true precisely
    because nothing checked this pair.
...
def test_an_image_with_no_owner_is_refused() -> None:
    """`ck_images_exactly_one_owner` in Python, and it has to be here as well as in the DDL.

    `replace_for_titles` assembles rows from a provider payload, and an ownerless one
    reaching Postgres is a `RepositoryConflict` that fails the whole derivation batch
    rather than the one row that is wrong.
    """
...
def test_an_image_with_two_owners_is_refused() -> None:
    """`= 1`, not `>= 1` -- the half a `num_nonnulls(...) >= 1` spelling would let through.
...
def test_each_owner_column_on_its_own_is_a_valid_image(owner: str) -> None:
    ...
    because a validator written as `title_id is None` reads correctly and refuses every
    episode still and every person headshot.
    """
...
def test_an_image_is_frozen_and_evolves_with_revalidation() -> None:
    """`DomainModel`'s standing rule.

    `.evolve()` re-validates, so a change that breaks the owner invariant raises here
...
def test_every_check_on_the_row_is_mirrored_as_a_field_bound(field: str, value: object) -> None:
    """This schema mirrors every Pydantic bound as a CHECK and the mirror runs both ways.

    `ck_images_provider_not_empty`, `ck_images_provider_path_not_empty`,
    `ck_images_width_positive` and `ck_images_height_positive` each have a field bound
    here, so the row a derivation assembles is refused at construction rather than by a
...
def test_a_dimensionless_image_is_ordinary() -> None:
    ...
    a provider that reports no dimensions and no language is ordinary, and a placeholder
    `0` is a lie a layout engine acts on.

    `gt=0` bounds the value when there is one and says nothing when there is not.
    """
...
def test_an_image_mints_its_own_id() -> None:
    """Identity is Usher's own UUIDv7, and the provider has no image id to borrow.

    which is the whole reason `(owner, provider, provider_path)` has to be the natural
    key rather than a provider integer.
    """

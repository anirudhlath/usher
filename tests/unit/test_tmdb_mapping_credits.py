...
def _creator(person_id: int, name: str) -> dict[str, Any]:
    ...
    and note what it does *not* carry: no `job`, no `order`, and **no
    `known_for_department`**, which is why `Person` declares that column nullable and
...
def test_a_series_creator_is_read_from_created_by_and_not_from_the_crew() -> None:
    """The ninth row of `mapping.py`'s per-kind divergence table.

    read out of the recorded fixtures rather than assumed.
...
def test_crew_carries_no_billing_order() -> None:
    """`crew[i]` has no `order` field -- read out of the fixtures, not assumed.

    So `billing_order` is meaningful for cast and `None` for crew.
...
def test_crew_outside_the_named_job_set_is_dropped() -> None:
    """Unfiltered crew is every gaffer.

    best boy and assistant art director, and *both* consumers of this table --
    `PeopleProvider`'s "more from this director" and weight class B -- want the people a
    viewer could name.

    Below the line, crews repeat because studios repeat, so an unfiltered set makes
...
def test_cast_beyond_the_billing_cutoff_is_dropped() -> None:
    ...
    A large film's `credits.cast` runs into the low hundreds; at the enriched tier
    boundary call 4 targets (2k-10k titles) an unbounded cast is roughly 10k x 150 ~
    1.5M credit rows against a database PRD 08 budgets at 8-12 GB *total*. At 50 it is
    ~500k.
...
def test_a_payload_with_no_credits_key_yields_no_people_and_no_credits() -> None:
    """A payload cached before `credits` joined `*_APPEND_TO_RESPONSE`.

    or an entity TMDb has none for.

    Zero people, zero credits, and **no error**: this is what most of the catalog looks
    like, not a fault.
    """
...
def test_belongs_to_collection_null_and_absent_are_the_same_outcome() -> None:
    """`null` is the ordinary case for a standalone film.

    the existing `RawPayloadStoreContract.PAYLOAD` fixture literally carries it -- and
    the key is **absent entirely** on every series, verified against `series.json`'s
    top-level key set.

    Both reach the same `payload.get(...)` and both mean "no collection", so a mapper
    that distinguished them would be inventing a state neither TMDb nor this schema has.
    """
...
def test_a_collection_with_no_usable_name_is_dropped_rather_than_raising() -> None:
    """`Collection.name` is `min_length=1` and a `pydantic.ValidationError` is **not** a.

    `UsherPortError`, so it would escape `JobWorker`'s except arms and take the process
    down rather than parking one job.

    Same standing rule `mapping.py` already states for `Title`.
    """
...
def test_a_cast_entry_with_no_id_is_dropped_rather_than_raising() -> None:
    """`mapping.py`'s standing rule.

    *nothing TMDb can put in a payload may raise*, because a `pydantic.ValidationError`
    is not a `UsherPortError` and would kill the worker instead of parking one job.
...
def test_one_person_credited_twice_on_one_film_is_two_credits() -> None:
    ...
    and `(title_id, person_id, kind, job)` is the natural key precisely so they do not
    collapse.

    A mapper that deduplicated on `(title_id, person_id)` keeps whichever it saw second
    and loses the other -- and `RecurringPerson` then reports a count that is right for
    the wrong reason.
    """
    people, credits = people_and_credits(
        _movie(
...
def test_a_person_on_both_the_cast_and_the_created_by_list_is_one_person() -> None:
    ...
    One `Person`, and the populated department wins -- an unconditional assignment
    blanks an actor's department the moment they also create a series.
    """
    people, credits = people_and_credits(
        _series(
...
def test_credits_name_the_title_they_were_derived_for() -> None:
    ...
    ADR-0003 identity, and the reverse lookup is the caller's because `raw_payloads` has
    no `title_id` at all.
    """

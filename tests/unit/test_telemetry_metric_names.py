...
**The counts are 34 and 35 and neither is a typo.** The catalogue has **35**
rows; the AST scan finds **34** declared instrument names; the difference is
...
form that cannot go stale silently.
...
anchor; the table parse must find 35 rows, which is the premise a Markdown
...
async def test_every_metric_name_usher_emits_is_a_row_of_prd_10s_catalogue(
    ...
    assert len(catalogue) == 35, f"the catalogue table parse found {len(catalogue)} rows"

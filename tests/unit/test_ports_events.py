...
def test_every_event_kind_is_something_this_process_emits() -> None:
    """PRD 10's rule for metrics, applied to a client contract.

    "A documented metric nothing emits is a dashboard panel that is permanently empty,
    and nothing distinguishes that from a healthy zero." An SSE event type nothing emits
    is worse -- a client writes a handler for it and waits forever.
    """
...
def test_an_event_may_be_scoped_to_a_title_or_to_nothing() -> None:
    ...
    woken by unrelated churn." A `sync.progress` event belongs to no title and must
    reach only unfiltered subscribers.
    """
    title_id = uuid.uuid4()
...
def test_an_episode_event_carries_its_series_title_for_filtering() -> None:
    ...
    it has no episode ids until it fetches the season.

    So an episode event carries both, and the filter matches on `title_id`.
    """
...
async def test_the_null_publisher_accepts_everything_and_does_nothing() -> None:
    """`usher work` as a standalone process publishes nowhere.

    and that has to be expressible without a branch in `EnrichService`.
    """
...
def test_publish_is_the_ports_only_method() -> None:
    """Subscription is not on the port.

    A Postgres `LISTEN/NOTIFY` implementation publishes with an `INSERT`/`NOTIFY` and
    subscribes with a dedicated connection whose lifecycle has nothing in common with an
    in-memory queue's -- putting both on one ABC would force the second implementation
    to satisfy a shape drawn around the first.
    """

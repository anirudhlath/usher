"""The canonical genre vocabulary (ADR-0039).
...
def test_the_two_source_spellings_of_science_fiction_share_one_canonical_label() -> None:
    """The visible half of issue #30.

    `Sci-Fi` is IMDb's and `Science Fiction` is TMDb's, and a viewer picking one of the
    two `/browse` facet buttons silently loses 6,223 or 20,051 titles.
    """
...
def test_expanding_a_filter_is_symmetric_in_the_spelling_the_client_sent() -> None:
    """A bookmarked `?genre=Sci-Fi` and a facet-driven `?genre=Science Fiction` are one query.

    over one population.

    Asserting **set equality of the whole expansion** rather than membership: an
    implementation that expanded only the canonical spelling passes a membership check
    on the canonical arm and still serves the legacy client half a concept.
    """
...
def test_a_fused_tmdb_tv_label_decomposes_into_both_concepts_it_names() -> None:
    ...
    and all three fused labels are in this catalog — `Sci-Fi & Fantasy` 165, `Action &
    Adventure` 154, `War & Politics` 25 (measured 2026-08-19).

    Collapsing one of them onto a single canonical label would delete the other half of
    what it says.
    """
...
def test_an_unknown_label_is_its_own_canonical_and_its_own_only_spelling() -> None:
    ...
    a third source, or a TMDb genre minted after this table was written, must filter
    exactly as it did before this change rather than vanishing from every answer.
    """
...
def test_canonicalising_a_title_s_labels_dedupes_and_keeps_first_seen_order() -> None:
    """What the facet collapse and `GenreNormalisationService` both need.

    and the dedupe is what makes the sweep idempotent: a title carrying `Sci-Fi &
    Fantasy` and `Sci-Fi` names Science Fiction once, not twice, so re-running the
    backfill over its own output is a no-op.
    """
...
def test_canonicalising_is_idempotent_over_the_whole_vocabulary() -> None:
    """**The property `usher genres --backfill` rests on.

    asserted directly rather than inferred from one example.**.
...
def test_folding_does_not_invent_a_match_for_a_label_outside_the_vocabulary() -> None:
    ...
    and this is the case that would catch a fold applied too early.
...
def test_no_two_labels_differ_only_by_case() -> None:
    ...
    so two labels differing only in capitalisation would collapse and one would silently
    shadow the other.

...
def test_the_prompt_states_the_bound_the_reader_refuses_a_completion_over() -> None:
    ...
    drift waiting to happen -- `MAX_QUERY_CHARS` is one definition crossing the prompt
    and the reader exactly as `min_cards` crosses curation's three.
    """
...
def test_every_declared_rule_reaches_the_prompt_in_order() -> None:
    ...
    boundary this file's docstring draws and states.
    """
...
def test_the_typed_query_is_rendered_as_one_line_whatever_whitespace_it_holds(
    ...
    r"""A viewer's query is third-party text going into a prompt.
...
async def test_a_bug_in_the_ledger_is_not_swallowed_as_an_upstream_failure() -> None:
    """`except UsherPortError` and not `except Exception`.

    A `TypeError` raised from this module is a bug, and a service that absorbed one
    would serve a plausible search forever while the cost ledger recorded nothing at
    all.
    """
    harness = _harness(
        {QUERY_KEY: "a crew alone in orbit"},
...
async def test_an_expansion_that_worked_warns_about_nothing() -> None:
    ...
    A warning on the path that *succeeded* is the per-request equivalent of the once-
    per-pass line `composition.metadata_provider` exists to have moved: it trains an
...
async def test_the_adapters_own_latency_wins_whenever_there_is_one() -> None:
    """The service's delta is a *fallback*.

    The adapter measures the request; this measures the whole attempt, prompt build
    included. Fails: a row that always reports the service's own number, which puts a
    value PRD 10 reads as request latency beside ones that are.
    """
    harness = _harness(
        {QUERY_KEY: "a crew alone in orbit"},
...
def test_the_clock_default_is_monotonic_rather_than_wall_clock() -> None:
    ...
    A behavioural assertion here (`latency_ms >= 0`) is itself one that cannot fail,
    because `_ms` clamps a negative delta to zero.
    """
...
def test_the_service_holds_no_repository_it_could_read_a_household_from() -> None:
    """A search box is not a household.

    Fails: an expansion that grew a `WatchStateRepository` to "personalise" a rewrite,
    which would put a viewing history on the wire once per search -- the cost and
    privacy shape `composition.llm_client`'s docstring warns an operator about once, at
    startup. An exact set rather than a subset check, so a collaborator cannot arrive
    without this list moving and somebody reading that sentence.
    """
...
def test_the_ledger_row_carries_the_id_type_the_table_is_keyed_on() -> None:
    """`llm_calls.id` is a UUIDv7 minted by `new_id()`.

    The port has no read and the dashboards group on `at`, so nothing downstream would
    notice a second row keyed differently until an insert conflicted.
    """

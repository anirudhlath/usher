...
def test_id_is_a_uuidv7() -> None:
    """Identity is Usher's own time-ordered id, never the dataset name."""
...
def test_evolve_revalidates() -> None:
    """Rules out `model_copy(update=)`, which skips validation and stores a bad position."""
    with pytest.raises(ValidationError):
...
def test_identifying_strings_cannot_be_empty(field: str) -> None:
    """An empty revision would compare equal across two different snapshots, splicing them."""
    with pytest.raises(ValidationError):
        _run(**{field: ""})


def test_timestamps_must_be_timezone_aware() -> None:
    """Timestamps are aware, matching Title: a naive heartbeat raises mid-import."""
    with pytest.raises(ValidationError):
...
def test_extra_fields_are_forbidden() -> None:
    """Extra fields are forbidden, so a row column with no matching field fails loudly."""
    with pytest.raises(ValidationError):
        _run(rows_skipped=3)
...
def test_status_has_no_rank_mapping() -> None:
    """ImportRunStatus is a status, not a ladder, so no rank map invites comparison."""

...
def test_id_is_a_uuidv7() -> None:
    """Same identity rule as every other entity (ADR-0003).

    Usher's own time-ordered id, never the dataset name.
    """
...
def test_evolve_revalidates() -> None:
    ...
    the latter skips validation and would happily store a negative position.
    """
    with pytest.raises(ValidationError):
...
def test_identifying_strings_cannot_be_empty(field: str) -> None:
    """An empty revision would compare equal to itself across two genuinely different snapshots.

    which is exactly the splice the revision guard exists to prevent.
    """
    with pytest.raises(ValidationError):
        _run(**{field: ""})
...
def test_timestamps_must_be_timezone_aware() -> None:
    """AwareDatetime, matching Title.

    A naive heartbeat compared against an aware one raises at runtime, in the middle of
    an import.
    """
    with pytest.raises(ValidationError):
...
def test_extra_fields_are_forbidden() -> None:
    """DomainModel's extra="forbid".

    ImportRunRow's columns are 1:1 with these fields, and _to_domain feeds every column
    in by name — a column added without a matching field must fail loudly there.
    """
    with pytest.raises(ValidationError):
        _run(rows_skipped=3)
...
def test_status_has_no_rank_mapping() -> None:
    """Deliberately unlike EnrichmentState (ADR-0008).

    which needs ENRICHMENT_RANK because comparing its members is a silent inversion.

    ImportRunStatus is a status, not a ladder: nothing ever asks "is this an
    improvement", so no rank map exists and adding one would invite the comparison it

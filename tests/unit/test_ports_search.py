"""The search port's DTOs.

The ABCs' behaviour is `tests/contract/ search_index_contract.py`; what is asserted here
"""
...
def test_a_search_document_carries_the_text_and_the_vector_the_service_assembled() -> None:
    ...
    `index(title_id)` forced a second engine to fetch each title back out to build its
    document -- 1.3M round-trips on a rebuild, against a service that was holding the
    `Title` already.
    """
    title_id = new_id()
...
def test_search_filters_are_a_closed_vocabulary_not_a_dict() -> None:
    ...
    `filters: dict[str, Any]` has no key vocabulary, so two backends invent different
    ones and nothing fails -- the second engine simply means something else by `owned`.
    """
...
def test_a_request_carries_the_query_vector_the_caller_computed() -> None:
    """Defect 4 of the 🔶, and the half that settles 🔶 3 as a side effect: the **caller** embeds.

    A backend doing its own embedding is a backend with its own model, its own prefix
    convention, and its own drift.
    """
...
def test_a_semantic_request_with_no_vector_is_refused_at_construction() -> None:
    """`SourceEvent.__post_init__`'s rule one port over.

    a DTO that can be built in a state no implementation can serve is a DTO that pushes
    the failure to whichever backend notices first.

    A `SEMANTIC` request with no vector has exactly two plausible readings -- "return
    nothing" and "embed it yourself" -- and the second is the one defect 4 exists to
...
def test_an_outcome_reports_semantic_coverage_beside_its_hits() -> None:
    ...
    vector rides back with the hits rather than being inferred.
    """
...
def test_every_search_dto_is_frozen_and_slotted(dto: type) -> None:
    """`ports/search.py`'s two original dataclasses were the only port DTOs in the repository.

    missing `slots=True`.

    A slotted dataclass refuses an attribute nobody declared, which is what makes a
    typo'd keyword a failure rather than a field that silently does nothing.
    """

...
def _legitimate_sources() -> dict[str, str]:
    ...
    The leak scan is a substring search, so a needle already inside one of these is
    indistinguishable from one that leaked, and the host arm below uses this to say
    so rather than to fail -- a machine named for its distribution is one
    `hostnamectl` away. **`git_sha()` is deliberately absent**: its value comes from
    git and cannot legitimately contain a hostname, so an implementation that put one
    there must fail the arm rather than excuse itself from it.
    """
    return {
...
def test_the_digest_is_stable_across_two_captures_of_the_same_catalog() -> None:
    ...
    Otherwise the baseline is invalid the moment it is written.
    """
...
def test_a_fingerprints_inputs_cannot_be_mutated_after_it_is_built() -> None:
    ...
    The digest is written to `eval.runs`, committed to `docs/evals/ledger.jsonl` and
    transcribed into `bars.toml`, and `ledger.py` reads `.digest` at two moments with
    a `session.commit()` between them. The first assertion is the copy and the second
    is the proxy, and neither subsumes the other: a `dict(...)` with no wrap leaves
    the fingerprint free to edit its own record, and a wrap with no copy leaves the
    caller holding the key to it.
    """
...
def test_the_digest_ignores_the_order_a_nested_mapping_was_built_in() -> None:
    ...
    `pools` is a nested mapping built one band at a time by whichever query answered
    first, so the two captures below differ in nothing a catalog can see and a
...
def test_the_digest_reads_the_keys_and_not_only_the_values() -> None:
    """A digest over `sorted(inputs.values())` passes every other case in this file.

    The ordering cases and the positive control are both satisfied by a function
    merely *sensitive* to the same numbers. The two captures here carry the identical
    multiset of values under swapped keys: 432 titles in a catalog of 81,054 shared
    names is not 81,054 titles in a catalog of 432.
    """
...
def test_any_one_input_field_moved_on_its_own_changes_the_digest(
    ...
    """The positive control above moves one field, so it cannot see a digest reading one.

    An implementation digesting `titles` alone -- or one that skips `pools` because a
    nested mapping is awkward to serialise -- passes it and reports a bootstrap
    re-run, an enrichment crawl and a re-sampled frame as all comparable with the
    baseline. The two integers and the nested mapping are the three shapes an input
    carries, each a parameter rather than an arm so the verdict names which field
    went unread.
    """
...
def test_a_field_that_is_absent_is_not_the_same_catalog_as_one_that_is_present(
    ...
    A digest built by reading known keys out of the mapping (`inputs.get(name)`), or
    one that drops `None`s on the way in, calls a capture that never read the pools
    identical to a capture that read them. The `None` parameter is the half that is
    not obvious: a frame whose pool query failed answers `None`, and "the pools are
    unknown" must not digest the same as "there is no pools field".
    """
...
def test_a_changed_git_sha_does_not_change_the_digest() -> None:
    ...
    Digested, that makes each run incomparable with the previous one,
    `baseline-invalid` the only reachable verdict, and the eval job noise that
    someone turns off.
    """
...
def test_no_provenance_field_of_any_kind_reaches_the_digest(moved: dict[str, object]) -> None:
    ...
    That is the repair somebody reaches for on being told the sha is the problem, and
    it re-creates the bug one field over: a `ranx` upgrade would then invalidate every
    baseline in the repository. The fourth parameter adds a key nothing recorded
    before, because a fixed key list cannot see a *new* provenance field arriving later.
    """
...
def test_the_recorded_provenance_is_read_from_the_run_and_not_written_down(
    ...
    """The case above cannot see a field whose value was *invented*.

    A recorded constant is worse than an absent record because a reader believes it.
    `"ranx": "0.3.21"` written as a literal is what the library reports today, so it
    agrees now and goes on agreeing after an upgrade -- at which point attributing a
    moved metric to a library rather than to the system under test quietly says the
    wrong thing. So both fields are asked for through the collaborator that answers
    them, made to answer something no literal would be.
    """
...
def test_the_digest_is_the_same_in_two_processes_with_different_hash_seeds() -> None:
    ...
    A digest over anything `PYTHONHASHSEED` salts -- `hash()`, a `set` iterated, a
    `frozenset` of band names -- agrees with itself all day inside one interpreter
    and with no other run of the harness, which every other case here is blind to.
    The digest's *shape* is asserted here because it is the same claim: 64 lowercase
    hex characters is what a sha256 answers and what a `hash()`, a truncation or a
    `repr` does not. `check=True` and the non-empty assertion are both load-bearing
    -- a child that failed to import prints nothing, and without them this compares
    `""` against `""` and calls it a pass.
    """
    digests = set()
    for seed in ("0", "1"):
...
def test_the_suggest_fingerprint_compares_the_frame_and_records_the_rest() -> None:
    ...
    A membership assertion (`"git_sha" in provenance`) is satisfied by a fingerprint
    that also digests it, so the halves are asserted whole and asserted disjoint,
    which is what a field *migrating* looks like rather than a field appearing.
    `seed` is an input rather than provenance, against the obvious reading: the seed
    selects which names were drawn, so two runs at different seeds cover different
    case sets. `case_count` is an input for the same reason. An embedding backfill
    moves `title_embeddings` and touches nothing here, which is the point.
    """
...
def test_any_one_number_of_the_suggest_frame_moving_changes_its_digest(
    ...
    Otherwise the run it describes is compared against a baseline drawn from a
    different catalog. The second parameter is a case count one transposition off
    rather than a round number, because that is the difference this field exists to
    notice.
    """
...
def test_the_gates_own_mappingproxy_pools_digest_as_the_plain_dict_spelling_does() -> None:
    """`for_suggest`'s `dict(frame.pools)` is load-bearing and is pinned by nothing else.

    Replacing it with `frame.pools` leaves every other case in this file green,
    because no fixture anywhere passes a non-`dict` `pools`. The state is one line
    away and mypy-clean -- `Frame.pools` is `Mapping[str, int]` and `GATE_POOLS` is a
    `MappingProxyType` -- and the failure is not a wrong number: `TypeError: Object
    of type mappingproxy is not JSON serializable` surfaces at `.digest`, as a crash
    out of the run rather than as a verdict. `Fingerprint.__post_init__` does not
    subsume this: it wraps the top level, and this proxy sits one level down inside
    the `pools` value.
    """
    proxied = for_suggest(
...
def test_the_gate_digest_is_the_digest_the_gates_own_constants_produce() -> None:
    """`GATE_DIGEST` is computed from the gate's four constants rather than transcribed.

    So it cannot drift from them, and this is the case that says the computation is
    still the one `for_suggest` performs. It pins that the constant is **in force**
    and cannot pin its *value* -- both sides move together if `_suggest_inputs`
    changes shape -- which is why the literal below is a second case rather than a
...
def test_the_gates_own_run_is_comparable_with_the_gates_baseline() -> None:
    ...
    Without it a `check_digest` that refused everything -- or one that refused
    nothing and never returned -- reads the same.
    """
    fingerprint = for_suggest(_GATE_FRAME, case_count=GATE_CASES)
...
def test_a_run_whose_inputs_are_not_the_gates_is_refused_and_the_input_is_named(
    ...
    """The hole this closes is reachable and quiet.

    `usher eval suggest --full --seed 12345` is a supported invocation. It yields a
    different digest, passes `check_frame` -- which sees `shared_lower_names` and the
    five pools and *cannot* see `surface`, `seed` or `case_count` -- is then judged
    against bars derived from `GATE_SEED`, and is written to `eval.runs` and to a
    git-committed `ledger.jsonl` with a `pass` or a `fail` beside it. The other three
    parameters are the remaining shapes an input takes, one per case so the verdict
    names which one went unchecked. The refusal names the moved input because an
    operator meets this in CI and cannot act on "the digest differs".
    """
...
def test_an_input_present_on_one_side_only_is_named_rather_than_merely_unequal(
    ...
    The first is the one a key-by-key comparison is most likely to lose: a run whose
    pool query failed carries `None`, and a surface that grew a field carries one the
    gate never had. A judge model id is an input by this module's own ruling, so the
    first judge run's fingerprint carries a key no suggest baseline has and must be
    refused as incomparable rather than digested into a silent mismatch.
    """
...
def test_two_input_mappings_only_the_serialisation_can_tell_apart_are_still_refused() -> None:
    ...
    which reads as a check that fired for no reason.
    """
    gate = dict(for_suggest(_GATE_FRAME, case_count=GATE_CASES).inputs)
...
def test_the_suggest_fingerprint_carries_no_credential_no_host_and_no_user(
    ...
    Anything that rides along in it is published.
    """
...
def test_the_sha_is_the_commit_git_names_for_the_tree_the_process_is_standing_in() -> None:
    ...
    `git rev-parse --short HEAD` or `git log -1` would answer a plausible-looking
    string that is not the sha a later reader would resolve. `monkeypatch.chdir` is
    not used and the comparison runs in `_ROOT` deliberately: `git_sha()` reads the
    *process's* working tree, which is what makes it usable from a container with a
    bind mount. The expected value is spelled out rather than assumed clean, because
    this case runs against whatever tree it is invoked on -- git is asked for the sha
    and asked separately whether the tree has moved past it, and the answer under
    test has to be exactly one of the two.
    """
...
def test_a_dirty_tree_is_marked_and_a_clean_one_is_not(
    ...
    """`git rev-parse HEAD` reads `.git/HEAD` and consults neither index nor worktree.

    On its own it records a clean sha for a tree carrying uncommitted code -- a sha
    naming code that did not run -- and that is the ordinary workflow rather than an
    edge case: `--full` appends to a git-committed `docs/evals/ledger.jsonl`, where
    an unmarked sha is permanent and wrong. Both arms run in a repository built here:
    the clean arm is what makes the dirty arm evidence, since an implementation
    appending `-dirty` unconditionally passes the dirty arm alone. The sha is
    asserted whole rather than by a suffix, because what precedes the marker has to
    stay a sha a reader can hand to `git show`.
    """
...
def test_a_tree_check_that_itself_failed_says_so_rather_than_reading_as_clean(
    ...
    `rev-parse` succeeded, so there is a sha; the tree check did not, so nothing was
    learned about the worktree. **Appending nothing would claim clean on evidence
    nobody obtained**, and appending `-dirty` is the mirror of it -- a permanent
    false marker on a baseline taken from a committed tree.
    """
    real = subprocess.run
...
def test_a_directory_that_is_not_a_repository_names_that_event_rather_than_crashing(
    ...
    """A run in a tarball, or in a container with no `.git`, is a legitimate run.

    Its provenance is simply thinner, and it is the *only* one of the three unknowns
    that is legitimate, which is why it is named apart from them rather than sharing
    a bare `"unknown"`.
    """
...
def test_a_git_that_cannot_be_run_at_all_names_which_way_it_failed(
    ...
    that expired. A harness that dies on either cannot run in a container, and one
...
def test_the_three_events_that_answer_no_sha_answer_three_different_things(
    ...
    """The claim the three cases above cannot make individually.

    Each of them pins a literal, and a pair of literals that had been made equal
    would pass both. All three are also asserted non-empty, because `provenance` is
    asserted truthy whole (`all(fingerprint.provenance.values())`) and an empty field
    reads in a report as a fact nobody had.
    """
    assert shutil.which("git"), "the premise: git is on PATH"
    monkeypatch.chdir(tmp_path)

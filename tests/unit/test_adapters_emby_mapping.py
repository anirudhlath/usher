...
def test_a_movie_maps_every_field_the_port_promises() -> None:
    """The audio assertions here are the recorded-payload half of the commentary trap.

    `movie_item.json`'s *first* audio stream is a two-channel AAC director's commentary
    and its second is the default TrueHD track, exactly as a real remux is laid out.

    Deleting `IsDefault` from that fixture -- or taking `MediaStreams[0]` -- turns
    `truehd`/8 into `aac`/2 right here, which the hand-built
    `test_the_default_audio_stream_is_preferred_over_the_first` below cannot do, because
    a hand-built dict is written by the same person who is asserting on it.
    """
...
def test_provider_id_keys_are_lowercased() -> None:
    """Emby spells them `Tmdb`/`Imdb`/`Tvdb`.

    M4's matcher reads `provider_ids["tmdb"]` and must not know that.
    """
...
def test_dolby_vision_wins_over_the_hdr10_fallback_layer() -> None:
    """The movie fixture carries the shape a real Dolby Vision file has on Emby 4.9.5.0.

    transcribed from the live server on 2026-07-31: `VideoRange: "DolbyVision"`,
    `ExtendedVideoType: "DolbyVision"`, and `ExtendedVideoSubType: "DoviProfile81"` --
    Profile 8.1, whose HDR10 base layer is genuinely there and genuinely not what the
    file is.

    Ordering the checks the other way round catalogues every DV file as HDR10.
...
def test_hdr_vocabulary(stream: dict[str, object], expected: HdrFormat | None) -> None:
    ...
    lossy, but true (HDR10+ carries an HDR10 base layer), and better than dropping the
    fact that the file is HDR at all.
    """
...
def test_every_dolby_vision_spelling_maps_to_dolby_vision(video_range_type: str) -> None:
    """Emby does not spell DV one way.

    `VideoRangeType` names the *base layer* alongside the DV marker -- `DOVIWithHDR10`,
    `DOVIWithHLG`, `DOVIWithSDR`, `DOVIWithEL` -- and only the bare `DOVI` is a value an
...
def test_the_four_shapes_emby_495_actually_emits(
    ...
    """Transcribed from the live server on 2026-07-31.

    and these four are the *whole* vocabulary it produced: every video stream of 200
    movies (the newest 100 4K and the newest 100 HD, out of 94,438) fell into one of
    them.
...
def test_a_dolby_vision_marker_in_the_extended_fields_wins_over_the_base_layer() -> None:
    ...
    What it emits is `ExtendedVideoType`/`ExtendedVideoSubType`, and neither was read
    here at all: the two fields the server actually populates were not in the vote.
...
def test_the_literal_string_none_is_not_a_video_range() -> None:
    """`ExtendedVideoType` and `ExtendedVideoSubType` carry the **string** `"None"` for an SDR.

    file, not JSON `null` -- verified live.

    So they are always truthy, and a check written as `if
    video.get("ExtendedVideoType")` would treat every SDR file in the library as
    carrying an HDR marker. Falling through a token table is what makes the string
    harmless.
    """
...
def test_a_dv_profile_wins_even_when_the_range_tokens_say_hdr10() -> None:
    ...
    "HDR"` and expose the Dolby Vision configuration only as `DvProfile`/`DvLevel`;
    without this check such a file is catalogued as HDR10, which is the HDR10 base layer
    that is genuinely present and genuinely not the point.
...
def test_audio_token_vocabulary(stream: dict[str, object], expected: str | None) -> None:
    """PRD 07's example value is exactly `truehd_atmos_7_1`.

    and this is where "the deep-link construction currently done by hand in the Home
    Assistant card moves here, where it is testable" becomes literally true.
    """
    assert audio_token(stream) == expected


def test_the_atmos_marker_is_read_from_the_stream_title_too() -> None:
    """Emby puts the feature vocabulary in whichever of `Profile` and `Title` the file happened.

    to carry it in -- a TrueHD track routinely has `Profile: "TrueHD"` and `Title:
    "Surround 7.1 Atmos"`, with the one word that decides whether a client can play it
    losslessly only in the title.

    Reading only `Profile` reports that track as plain `truehd_7_1`, which is PRD 07's
    `truehd_atmos_7_1` example silently downgraded.
    """
    assert (
...
def test_only_the_first_matching_feature_is_appended() -> None:
    ...
    and both rows of the feature table map to the same token -- so without the first-
    match-wins break the token is `dts_hd_ma_hd_ma_7_1`.

    Emby emits exactly this string as an audio `Profile`, so this is a real payload, not
    a constructed one.
    """
    assert (
...
def test_a_series_has_no_media_and_no_runtime() -> None:
    """`RunTimeTicks` is literally `null` in the fixture.

    and there is no `MediaSources` key at all -- both are how Emby describes a folder,
    and both are places a mapper that assumed a value would raise.
    """
    item = to_source_item(load_emby_fixture("series_item"))
    assert item is not None
...
def test_an_unmodelled_item_type_is_skipped_not_raised() -> None:
    """Seasons, box sets, and playlists come back from a server that ignores `IncludeItemTypes`.

    Skipping keeps the walk going; raising would abort a 94,395-item reconcile over a
    box set.
    """
...
def test_an_item_with_no_id_is_malformed() -> None:
    """Distinct from an unmodelled type.

    an item with no id cannot be upserted on `(source_id, external_id)`, so silently
    skipping it would lose a real item with no trace.
    """
...
def test_missing_user_data_is_not_a_zero_state() -> None:
    ...
    `UserData` is absent when the field was not requested; emitting a zero state for
...
def test_a_timestamp_without_an_offset_is_still_aware() -> None:
    ...
    `fromisoformat` returns a *naive* datetime for a value with no offset, and
    `SourceItem` is a plain dataclass that would carry it happily all the way to a
    TIMESTAMPTZ insert.

    Emby's timestamps are UTC, so the offset is attached rather than the value rejected.
    """
...
def test_a_cursor_is_widened_by_one_second() -> None:
    """The port promises `since` is inclusive.

    whether Emby's own comparison is `>=` or `>` is unverified.
...
def test_a_sub_second_cursor_widens_by_up_to_two_seconds_not_one() -> None:
    """The widening is one second *plus* whatever sub-second part is truncated by the.

    whole-second format Emby's date parameters take, so the real bound is one to two
    seconds -- 1.9 s here.

    Stated rather than fixed: the direction is the safe one (a wider window returns a
    superset, which the port permits), and the cursors this is called with come from
    `SourceItem.added_at`, which carries Emby's own sub-second precision. Neither the
    fake server nor a real one can expose this, because both compare these strings at
...
def test_a_naive_cursor_is_refused_rather_than_silently_shifted() -> None:
    """`AwareDatetime` is a bare annotation on a plain function pydantic never validates.

    so a naive datetime went straight through -- and `astimezone` then reads it in the
    *host's* local zone.

    Measured on this machine (UTC-5): a naive `12:00` became
    `MinDateLastSaved=2026-07-20T16:59:59Z`. Five hours of changes skipped, in exactly
    the direction the deliberate one-second widening above exists to avoid, and eighteen
...
def test_the_default_audio_stream_is_preferred_over_the_first() -> None:
    """A file whose first audio track is a commentary and whose default is the feature audio is.
...
def test_with_no_default_flag_anywhere_the_first_stream_is_used() -> None:
    """Plenty of files flag no stream as default at all.

    a remux whose muxer never wrote the disposition bit.

    Returning `None` there would report a perfectly ordinary file as having no audio;
    the first stream is the same choice every player makes.
    """
    media_source = {
        "MediaStreams": [
...
def test_a_boolean_in_a_numeric_field_does_not_become_a_year() -> None:
    """The consequence `as_int`'s bool guard exists for, at the payload level.
...
def test_an_item_with_no_name_falls_back_to_its_external_id() -> None:
    """`SourceItem.name` is not optional.

    and Emby will answer with an item whose `Name` is absent or empty (a stub row for a
    file it has not yet probed).

    The id is a poor name and a perfectly good one to render in a dashboard; `None`
    would fail much later, at a NOT NULL column.
    """
...
def test_item_level_dimensions_are_the_fallback_when_the_stream_has_none() -> None:
    """Emby sets `Width`/`Height` on the item for some libraries and only on the video stream.

    for others -- which of the two a given server uses is not something this adapter
    gets to choose.
    """
...
def test_a_malformed_item_reports_a_truncated_name_not_a_payload_dump() -> None:
    """`PortDataMalformed.detail` "must never carry a credential or a whole payload".

    An Emby item name is operator-controlled and unbounded, and this string is built to
    be logged -- the same log-hygiene control as the credential guards in `EmbySession`,
    applied to the one field here that a payload can make arbitrarily long.
    """
...
def test_a_multi_version_item_is_catalogued_at_its_best_playable_version() -> None:
    ...
    "first wins" catalogues a version with no container at all, "last wins" catalogues
    the 1080p one.
...
def test_a_transcode_only_item_is_still_catalogued() -> None:
    """The fallback half of the rule.

    An item Emby can only transcode has no container, so there is no direct URL to build
    for it -- but it still has a real codec, resolution and runtime, and reporting
    `None` for all of them would put a hole in the catalogue over a playback limitation.
    """
...
def test_a_media_sources_entry_that_is_not_an_object_is_skipped() -> None:
    """`MediaSources` is a list of objects, until a server answers with a list of something else.
...
def test_provider_ids_drops_entries_with_nothing_in_them() -> None:
    ...
    and M4's matcher looks titles up by `provider_ids["tmdb"]`, so an empty value that
    survives here becomes a lookup for the empty id rather than a fallback to name
    matching.
    """
...
def test_a_negative_playback_position_and_play_count_are_clamped() -> None:
    """Floor division makes a negative worse, not better.

    `-1 // 10_000_000` is `-1`, so an out-of-range tick count arrives as a negative
    `position_seconds` in a `SourceWatchState` -- a plain dataclass that validates
    nothing -- and fails at a CHECK constraint several layers later, where nothing left
    in scope says which item it came from.
    """
    state = to_watch_state(
...
def test_raw_shares_no_object_with_the_payload_it_was_parsed_from() -> None:
    """PRD 03 stores `raw` verbatim in `raw_payloads`.

    It is the one field that crosses the adapter boundary uninterpreted, so it must not
    be a live view of a buffer the adapter is still reading: `get_item` parses one
    payload and passes it to *both* `to_source_item` and `build_stream_targets`.
...
def test_a_listing_payload_yields_absent_play_history() -> None:
    """Emby 4.9.5.0's listing route reports `PlayCount.

    0` and omits `LastPlayedDate` for items that have genuinely been played (verified
    2026-07-31).

    Passing that `0` through as a number is how M4 would write zero over real history,
    so the mapper must report absence instead.
    """
    payload = {
        "Id": "movie-1",
        "Name": "Example Movie",
...
def test_a_listing_payload_discards_play_history_even_when_it_carries_some() -> None:
    ...
    A build (or a reverse proxy, or a future Emby) whose listing did carry `PlayCount`
    would still be read through a caller that cannot tell whether the number is real --
    and the measured server hands out a `0` that looks exactly like a count. Discarding
    is the only rule that is safe for both, and `get_watch_state` is what recovers the
    truth.
...
def test_an_item_payload_yields_real_play_history() -> None:
    """The single-item route does carry both.

    and that is the whole reason `get_watch_state` exists.
    """
    payload = {
        "Id": "movie-1",
        "Name": "Example Movie",
...
def test_a_trusted_payload_that_reports_zero_plays_is_believed() -> None:
    """The other half of ADR-0014: `0` from a route that can count is a positive claim.

    an item whose play history was reset -- and turning it into `None` would make a
    reset impossible to propagate, which is the same correctness bug as filtering all-
    zero states out of a walk.
    """
    payload = {
        "Id": "movie-1",
        "Type": "Movie",

"""`usher.__main__` is the container entrypoint (`python -m usher`).

see its own module docstring for why it exists instead of the plain `uvicorn` CLI
"""
...
def test_main_defaults_match_dockerfile_expose(monkeypatch: pytest.MonkeyPatch) -> None:
    """No USHER_HOST/USHER_PORT set.

    the defaults Settings falls back to must match what the Dockerfile EXPOSEs and what
    compose.yml's usher healthcheck probes, or a default deployment silently mismatches.
    """
    monkeypatch.setenv("USHER_DATABASE_URL", "postgresql+asyncpg://u:p@db:5432/usher")
    monkeypatch.setenv("USHER_SECRET_KEY", "s" * 32)
...
def _dispatched(monkeypatch: pytest.MonkeyPatch, argv: list[str]) -> list[str]:
    """`sys.argv` set as a shell would set it.

    then `main()` called the way a `[project.scripts]` console script calls it -- with
    no arguments.
...
def test_the_console_script_dispatches_on_sys_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    """`[project.scripts] usher = "usher.cli:main"` calls `main()` with **no arguments**.

    so `main` has to read `sys.argv` itself.
...
def test_the_console_script_with_no_arguments_still_serves(
    ...
    """The property `python -m usher` has always had, now shared by both entry points.

    no arguments means run the server, because that is what the container's `CMD` does
    (`alembic upgrade head && exec python -m usher`).

    Adding a console script must not change it.
    """

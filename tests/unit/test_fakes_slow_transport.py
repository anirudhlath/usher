...
async def test_a_streaming_request_body_is_read_before_the_handler_sees_it() -> None:
    """`httpx.MockTransport` awaits `request.aread()` before calling its handler.

    this one did not.

    `FakeEmbyServer._authenticate` reads `request.content`, which raises
    `httpx.RequestNotRead` for a request whose body has not been read -- latent only
    because `EmbySession` happens to build every request with `json=`, which is already
    bytes.
    """
    seen: list[bytes] = []
...
async def test_a_cancelled_request_does_not_leak_the_in_flight_counter() -> None:
    """The counter is the instrument the single-flight tests read to prove they forced real.

    concurrency, and it could only ever lie *upward*: the increment sat before the `try`
    and the `await asyncio.sleep` sat outside it, so a cancellation during the sleep
    skipped the `finally` and left `in_flight` permanently high.

    Every later request then read a `max_in_flight` inflated by a request that was not
    actually in flight -- the instrument failing in exactly the direction that certifies
    a test as concurrent when it has stopped being so.
    """

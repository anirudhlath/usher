"""Schema shape for the three bootstrap tables.

No Docker: these read SQLAlchemy metadata, not a live database.
"""
...
def test_import_run_row_is_one_to_one_with_the_domain_model() -> None:
    """The standing constraint TitleRow/Title already hold to.

    _to_domain feeds every column into model_validate by name under extra="forbid", so a
    column with no matching field is a runtime ValidationError, not a silently dropped
    value. Adding one here means adding a field there.
    """
...
def test_import_runs_is_keyed_by_dataset() -> None:
    """One row per dataset, updated in place — a checkpoint, not a log.

    The unique constraint is what stops a second concurrent bootstrap of the same
    dataset from quietly creating a rival cursor.
    """

...
def test_a_frame_carries_an_id_an_event_and_a_json_data_line() -> None:
    """The three fields an `EventSource` reads.

    `id:` is what comes back as `Last-Event-ID`, so the epoch has to be in it -- the
    bus's whole unknown-epoch branch is unreachable otherwise.
    """
    frame = encode_sse(
...
def test_a_title_id_is_on_the_data_line_not_only_in_the_filter() -> None:
    ...
    The `?titles=` filter decides *whether* a frame is sent, not what it says.
    """
    title_id = uuid.uuid4()
...
def test_every_internal_kind_has_a_wire_name() -> None:
    """The mapping is total in the direction that matters.

    A `ClientEventKind` with no wire member is a `KeyError` in the middle of a stream
    that has already answered 200 -- there is no status code left to say so with, which
    is exactly why the SSE vocabulary is pinned as an enum rather than deferred to PRD
    07's problem-details envelope.
    """
...
def test_a_row_invalidation_carries_no_title_id() -> None:
    """A row is not a title.

    `title_id` on this event would be a filter key that half-works -- it would wake
    exactly the detail screens subscribed to whichever title happened to be attached,
    which is neither "every subscriber" nor "the right ones".
    """
    frame = encode_sse(

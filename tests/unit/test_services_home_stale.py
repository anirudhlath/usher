...
class _Clock:
    ...
    Copied rather than imported: a suite whose expiry boundaries depend on a
    sibling file's internals breaks for reasons unrelated to its subject.
    """
...
def _without_suspending(coro: Coroutine[Any, Any, Any]) -> Any:
    ...
    A coroutine that reached any suspension point hands back a future here
    instead of raising `StopIteration`, and it does so *deterministically* --
    where a case that awaited a refresher parked on a gate could only report a
    timeout, which is what a slow box also looks like. Closed on the failure
    path so the failure is one assertion rather than one plus a "coroutine was
...
async def test_a_stale_screen_is_served_without_waiting_for_the_refresh(
    ...
    """The clock stepped *onto* `_SCREEN_TTL`: expired, inside the grace window.

    Nothing drains the refresh queue, so the refresh that gets scheduled can
    never complete during this case. An implementation that pops the expired
    entry makes the request pay a full compose; one that awaits the refresh
    fails at `_without_suspending`, by construction rather than by timing out.
    """
...
async def test_past_the_grace_window_the_entry_is_a_hard_miss_and_is_never_served(
    ...
    `_SCREEN_TTL + SCREEN_STALE_GRACE` is the instant a stale entry stops being
    servable, and `>=` versus `>` there is a one-keystroke mutation invisible to
    every case that steps past. A hard miss rebuilds, and it must *not* also
    schedule a refresh: the request already paid for the rebuild, so a scheduled
    one is a second full compose for a screen that is now fresh.
    """
    cache = RowCache(clock=clock)
    queue = RefreshQueue()
    provider = _provider()
...
async def test_a_composer_with_no_refresher_never_serves_stale(
    ...
    A composer that opened the window with nothing behind it would serve a stale
    screen and never replace it -- strictly worse than the miss it avoided, and
    silent. `usher home` is that caller: the process ends when the command does.
    Kills a grace window applied unconditionally, which every other case in this
    file passes because every other case injects a refresher.
    """
    cache = RowCache(clock=clock)
    provider = _provider()
...
async def test_two_reads_over_one_stale_key_schedule_one_refresh(
    ...
    """Dedup at the queue's level, and the second read is still served stale.

    The second half is not decoration: `read_screen` returning the stale value
    *and popping the entry* satisfies every other assertion in this file, yet
    leaves the next request cold, so the household pays the full compose
    serve-stale exists to spare it. This is a count, not a concurrency claim --
    it rules out a queue with no dedup at all; that a request arriving while a
    refresh is *in flight* schedules nothing needs observed overlap and lives in
...
async def test_a_full_queue_drops_the_key_and_the_request_still_does_not_wait(
    ...
    fills. Dropping is safe rather than merely tolerable: the entry is still
    inside its grace window, and once past it the next request rebuilds.
    """
    cache = RowCache(clock=clock)
...
async def test_rebuild_ignores_the_cached_screen_and_replaces_it(
    ...
    """`rebuild` is a *different* method from `compose` for one reason.

    A refresh that went through the ordinary read would find its own stale
    entry, serve it to itself, and schedule another refresh -- a lane spinning
    on one key forever, with the household still looking at the same stale
    screen. The row cache underneath is still consulted: past the screen's TTL
    the row's own has not moved, so the refresh re-proposes and re-orders
    without re-hydrating, where dropping the row half would make every screen
...
async def test_a_stale_serve_costs_no_taste_read(clock: _Clock) -> None:
    """A screen the cache can answer never pays the statements behind `affinities`.

    `RowContext.affinities` is a callable for that reason (`api/deps.py`
    `_Affinities`). A *stale* serve is a screen the cache answered, so it owes
    them no more than a fresh one does, and a serve-stale path that resolved the
    context eagerly would put the whole genre-affinity read back in front of the
...
def test_the_queue_hands_over_the_user_the_request_already_resolved() -> None:
    ...
    The context holds repositories bound to the request's `AsyncSession`, which
    `get_session` commits and closes when the handler returns -- so a queue
    carrying one would hand a background task either a closed session or a live
    one shared with a request, a concurrency hazard that *usually works*. The
    `User` is immutable and is what the refresh needs to build its own context.
    """
    queue = RefreshQueue()
...
def test_a_screen_stale_by_a_negative_ttl_is_still_inside_the_grace_window() -> None:
    ...
    Pinned here so that file's premise is not an assumption about arithmetic it

...
def test_title_with_no_media_item_is_legal() -> None:
    """Most of the catalog after bootstrap is titles nobody owns yet.

    a Title needs no MediaItem to exist.

    Pinned explicitly so a future validator that assumes otherwise (e.g. requiring at
    least one MediaItem) fails CI instead of silently shipping.
    """
...
def test_media_item_rejects_unknown_hdr_format() -> None:
    """A source's raw HDR string (Emby emits "DolbyVision") must not reach this field directly.

    its adapter is responsible for translating it into HdrFormat before MediaItem ever
    sees it.
    """
    with pytest.raises(ValidationError):
        MediaItem.model_validate(

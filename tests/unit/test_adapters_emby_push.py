...
def test_an_open_connection_that_has_delivered_nothing_is_not_delivering() -> None:
    """**The rule this whole milestone exists for.** ADR-0004.

    a handshake against a *nonexistent path* also upgrades and also receives `Sessions`,
    so a successful upgrade is not evidence of anything.
...
def test_silence_is_measured_from_the_last_message_or_from_the_open() -> None:
    ...
    channel that never delivers must still become measurably silent -- otherwise the one
    failure mode this milestone is built around is the one the watchdog cannot see.
    """
    health = PushHealth(stale_after=90.0)
    health.opened_at = 100.0
...
def test_silence_before_a_connection_is_zero_rather_than_undefined() -> None:
    ...
    so this branch is unreachable in production -- and a `None` minus a float is a
    `TypeError` that would take the lane down rather than reconnect it.

    Pinned directly, the way M4 pins its two other unreachable-through-the-contract
    guards.
    """
...
def test_a_first_connection_is_not_a_reconnect() -> None:
    ...
    and the off-by-one that would put every source's dashboard panel at 1 from start-up.
...
async def test_the_channel_counts_its_own_reconnects() -> None:
    ...
    and until this, nothing in `src/` ever did: `record_reconnect` was a method with no
    caller, so PRD 10's reconnect series would have plotted a flat zero for every source
    forever.
...
def test_a_carried_state_reports_position_and_played_and_nothing_else() -> None:
    """ADR-0014 on a third payload shape.

    `PlayCount: 3` and a `LastPlayedDate` are both present in the fixture and both
    ignored: no run in this repository has ever parsed a real `UserDataChanged`, and a
    number reported from a shape nobody has measured is exactly the claim that rule
    forbids. The `watch_history` backfill recovers the pair from the single-item route,
    which is the chain M4 already built and measured.
    """
    events = to_source_events(load_emby_fixture("push_user_data_changed"), source_user_id="u1")
...
def test_a_sessions_message_becomes_nothing() -> None:
    """**And that is its whole job.** `Sessions` carries playback state for sessions Usher is.

    not part of; deriving anything from it would mean tracking play sessions Usher never
    starts.
...
def test_a_user_data_list_that_is_not_a_list_is_dropped_not_raised() -> None:
    """`PortDataMalformed` from a push mapper would park nothing (there is no job) and would.

    take the lane down through `PushSupervisor`'s `UsherPortError` arm, so one malformed
    frame would cost a reconnect and a gap-closing delta.
...
def test_a_user_data_list_of_entries_that_are_all_unkeyable_becomes_nothing() -> None:
    ...
    and it is the one that would construct a `SourceEvent` naming no items:
...
def test_a_state_carries_no_user_when_the_source_did_not_distinguish() -> None:
    """`source_user_id=None` is passed straight through rather than being filled with a.

    placeholder -- `SourceWatchState` documents it as "the source didn't distinguish",
    and a made-up id would become a real column value on `watch_states`.
    """
...
async def test_the_channel_subscribes_with_adr_0004s_own_frame() -> None:
    ...
    Without it Emby holds the socket and sends nothing -- which is the exact upgraded-
    but-silent state this milestone is built around, arrived at by forgetting one line.
    """
    connector = FakePushConnector()
...
async def test_the_socket_url_carries_the_token_and_the_device_id() -> None:
    ...
    `DeviceId`; ADR-0004 read both out of `SessionWebSocketListener` and they are
    genuinely different.
    """
...
async def test_the_socket_url_percent_encodes_both_values() -> None:
    ...
    and `&`/`=`/`?` in either would otherwise re-shape the query.
...
async def test_the_channel_takes_its_token_from_the_session_on_every_open() -> None:
    """PRD 03's durable-client property comes from authenticating *once* with a stable.
...
async def test_the_channel_counts_every_frame_including_ones_it_maps_to_nothing() -> None:
    """A `Sessions` message produces no event and is the *reason* an idle library's channel.

    stays measurably alive.
...
async def test_an_already_translated_connect_failure_propagates_unchanged() -> None:
    """The plan asserted `WEBSOCKET_PATH in str(...)` against this shape and it is not true.

    a `PortUnavailable` from the connector is re-raised as it stands, so the message is
    the connector's.
...
async def test_a_send_that_fails_does_not_leave_the_connection_open() -> None:
    ...
    so a failure there is the one path that could leave a ledger reporting `connected`
    on a socket nobody is holding -- which is the milestone's own failure mode, reached
    from inside.
    """
    connection = FakePushConnection()
...
async def test_the_health_ledger_is_the_one_the_caller_handed_in() -> None:
    ...
    so `reconnects` and `messages_received` are the lane's history rather than one
    connection's.
    """
    health = PushHealth(stale_after=90.0)
    connector = FakePushConnector()
...
async def test_a_channel_inside_the_window_keeps_waiting() -> None:
    ...
    which turns every idle second into a reconnect and a gap-closing delta walk against
    a server PRD 01 measures at 1-5 s per request.
...
async def test_the_channel_polls_with_its_own_timeout() -> None:
    ...
    that is what makes a staleness test sub-millisecond instead of ninety seconds -- so
    the value passed is observable only because the fake records it.
...
def test_the_socket_logger_is_re_silenced_on_every_call() -> None:
    """`configure_logging` walks `logging.root.manager.loggerDict` and sets `propagate = True`.

    on everything it finds, so a `propagate = False` set once at import is undone the
    next time an app is built -- and the test suite alone builds dozens.
...
def test_the_socket_logger_is_not_one_configure_logging_can_find_by_name() -> None:
    ...
    but it is still an ordinary stdlib logger, and the guarantee is that its *level* is
...
async def test_connect_websocket_hands_the_library_the_silenced_logger(
    ...
    """The one argument in `connect_websocket` that is a security control rather than a tuning.

    knob, pinned where it is passed.
...
async def test_a_real_websockets_handshake_prints_no_token(
    ...
    a real `websockets` client, a real `websockets` server on `127.0.0.1`, a real
    upgrade, and `configure_logging` at DEBUG -- the exact configuration that puts the
    token on stdout in `test_the_stock_websockets_logger_really_does_print_the_token`
    above.
...
async def test_the_real_connection_translates_a_closed_socket_by_type(
    ...
    """`_WebsocketsConnection` exists so that **no `websockets` exception ever crosses into.
...
async def test_the_real_connection_never_raises_out_of_aclose() -> None:
    """`aclose` runs in a `finally` that is itself often unwinding the `PortUnavailable` that.

    explains why the lane dropped.

    A close failure replacing that reason is worse than a log line, so it is swallowed

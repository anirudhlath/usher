...
#: cases below assert the **whole** mapping.
_NO_LANES = {
    "push": [],
    "worker": False,
    "crashed_sources": [],
    # `null`, never `0`: this process runs no worker, so it never called
    # `recover()` -- it has not asked.
...
async def test_a_process_with_no_lanes_running_is_still_ready(client: AsyncClient) -> None:
    """A readiness check must not fail because no push lane is up.

    That would take this process out of a load balancer for a reason restarting it
    cannot fix, and PRD 08's own failure table says an unreachable source leaves the
    catalog "fully browsable". It has to be *here* rather than in
    `tests/unit/test_api_health.py`, because that file's app points at an unreachable
    database and is therefore already 503: `all(checks) and lanes.running_sources()`
    and `push` moved inside `ReadinessChecks` both survive there. Against a reachable
    database with no lanes running, both turn a 200 into a 503 and both die.
    """
    response = await client.get("/health/ready")
    assert response.status_code == 200
    body = response.json()
...
async def test_a_third_lane_kind_changes_neither_the_status_code_nor_the_report(
    ...
    """A third lane kind changes neither the status code nor the report.

    The `rows.refresh` lane is running in this very process -- it is gated on
    `create_app` building a cache and a queue, not on a setting, so it is up even
    with `push_enabled=False, worker_enabled=False`. Two mutations die here and
    nowhere else: reporting it inside `ReadinessChecks`, where
    `all(checks.model_dump().values())` picks it up automatically and a screen
    refresh starts deciding whether a load balancer sends traffic; and folding it
    into `running_sources()`, which would then name something that is not a source.
    Here rather than in `tests/unit/test_api_health.py`, for the reason the case
    above gives.
    """
...
#: only way to own `jobs.updated_at`: every statement in `PostgresJobQueue` stamps
#: it `clock_timestamp()` itself, so a row enqueued and claimed through the port is
#: by construction fresh and can never be older than the lease.
_PLANT_AN_ORPHAN = """
...
async def clean(sessions: async_sessionmaker[AsyncSession]) -> AsyncIterator[None]:
    ...
    A run that died between the two would otherwise leave a `running` row that makes
    the next run's count wrong.
    """
...
async def test_a_recovered_orphan_is_reported_in_the_body_and_moves_no_status_code(
    ...
    """A recovered orphan is reported in the body and moves no status code.

    A worker died holding claims and another worker took them back; an operator
    watching `/health/ready` saw `worker: true` throughout and nothing else.
    """
...
async def test_a_worker_that_asked_and_found_nothing_reports_zero_not_null_and_not_one(
    ...
    `null` / `0` / non-zero are three different statements -- never asked, asked and
    found none, took some back -- and the two cases beside this one pin the first and
    the third. Without this one, a counter incremented *before* `recover()` rather
    than from its **return value** reports `1` for a pass that recovered nothing and
    `1` for a pass that recovered one, and neither neighbour can tell them apart.
    `recovered_at` staying `null` is the other half: it is the instant of the last
    pass that **found** something, not of the last pass.
    """
    async with sessions() as session:
...
async def test_check_migrations_detects_a_mismatch(session: AsyncSession) -> None:
    """Uses the transaction-isolated `session` fixture directly, not `app`/`client`.

    Corrupting `alembic_version` needs to be visible to the very next query on the
    *same* connection, not committed and visible cross-connection to the app's own
    separately-built engine -- and the rollback this fixture does afterwards leaves
    the shared, session-scoped database exactly as every other test expects it.
    """

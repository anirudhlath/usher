...
async def test_the_frame_counts_exactly_what_the_pools_return(
    ...
    Spelled twice they would agree today and drift the first time either was edited --
    and a frame check over a different population than the draw is a check of nothing.
    """
...
async def test_every_band_is_present_even_when_empty(session: AsyncSession) -> None:
    """An absent band and an empty one are different facts.

    A generator that dropped empty bands would silently produce a smaller case set under
    the same seed.
    """
    pools = await read_pools(session)
...
async def test_the_pools_are_ordered_by_id(session: AsyncSession) -> None:
    """`random.Random.sample` draws by position.

    so the order the rows arrive in *is* part of the seed.

    An unordered read makes two runs of the same seed different measurements.
    """
    pools = await read_pools(session)

        ...
        await session.execute(
            text(
                ...
                SELECT count(*) AS titles,
                       ...
                       -- this run saw is what decides whether a refused
                       -- frame is a stale gate or an unimported dump.
                       count(*) FILTER (WHERE imdb_num_votes IS NOT NULL)
                           AS with_imdb_num_votes,
...
async def keystroke_probes(session: AsyncSession, chooser: random.Random) -> list[tuple[int, str]]:
    ...
    rows = (
        await session.execute(
            text(
                "SELECT t.name FROM titles t WHERE t.kind = 'movie' AND t.imdb_num_votes >= 500 "
...
# **W3's second configuration, and the reason it exists is that the first one flatters
# the union.** The worst-case plan is a Gather with two workers (`loops=3`), so a one-
# character union probe is being timed *with two spare cores thrown at it on an
# otherwise idle box* -- and a type-ahead surface is concurrent by definition.
...
async def measure_tier1(
    ...
    *,
    ...
    if worst:
        ...
        # **The knob is verified to have landed, not assumed.** `SET LOCAL` on a GUC
        # that does not exist raises, but one that exists and is scoped to the wrong
        # transaction succeeds and changes nothing -- and a serial figure that is really
        # a second parallel figure is indistinguishable from a finding that parallelism
...
def _verdicts(log: RunLog) -> dict[str, Any]:
    ...
    def _bar1(source: dict[str, Any], key: str) -> dict[str, Any]:
        ...
        return {
            ...
            # **Both, side by side, never one replacing the other.** The parallel figure
            # is true of an idle box and is the honest ceiling; the serial figure is the
            # floor a busy box gives you.
    ...
    if ab:
        ...
        # **Within-run, over the identical 2,993 cases, with only the two indexes
        # differing.** Comparing today's p50 to the gate's 33.6 ms is a comparison
        # across two days, two runs and two levels of box contention; this one has a
        # single variable.
...
async def run(args: argparse.Namespace) -> None:
    ...
    # **Absolute, because a box that got *quieter* mid-run is also a box that was not
    # the same box throughout.** A sibling finishing halfway through means the first
    # half was contended, which corrupts a p95 exactly as much as a sibling starting
    # halfway through does.

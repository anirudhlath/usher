...
the measurement is.** Movies only, `imdb_num_votes >= 500`, names not
unique in the
...
🔴 **The predicate was `tmdb_vote_count >= 500` for one release and that was
the wrong half of ADR-0040's split.** The 2026-08-03 gate drew from
`titles.vote_count`, a column two writers filled -- IMDb's `numVotes` on every
bulk-loaded row and TMDb's `vote_count` on every enriched one -- and the split
gave the IMDb half to `imdb_num_votes`. This script was renamed onto the TMDb
half, mechanically, so it kept parsing and changed electorate. Measured
read-only against the live 1,276,210-title catalog on 2026-08-27:

| band | gate, 2026-08-03 | `tmdb_vote_count` | `imdb_num_votes` |
|---|---|---|---|
| 2-4 | 432 | **80** | **435** |
| 5-7 | 2,532 | **440** | **2,544** |
| 8-11 | 7,178 | **1,315** | **7,220** |
| 12-19 | 20,520 | **3,349** | **20,675** |
| 20+ | 17,887 | **3,323** | **18,195** |

An order of magnitude out on every band under the shipped spelling and within
**1.7%** on every band under the split-out one -- which is the same repair, and
the same size of confirmation, `usher eval suggest`'s own frame got when
ADR-0040 took its sampling frame from 48,549 to 8,523 and re-anchoring
answered 48,639.
...
⚠️ **So `--check-frame` still refuses on this deployment, and that is the
correct answer rather than a residual defect.** The catalog has grown since
2026-08-03: 81,369 shared lower-cased names against 81,054, and the five pools
in the right-hand column above. `check_frame`'s own docstring is why the
numbers are not simply updated -- *"a pool that is one row out is a different
eligible population, and a recall figure taken over a different population is
not the gate's figure however close it looks"*. **`GATE_POOLS` is the gate's,
not today's**, and rewriting it to whatever the catalog currently holds would
convert a check that can say *"the frame moved"* into one that can only ever
say *"the frame is whatever it is"*. What the re-anchoring buys is a refusal
whose delta is legible -- 435 against 432 reads as three films arriving, where
80 against 432 read as nothing in particular. Anyone needing the gate's exact
population needs the gate's catalog; anyone needing a *comparable* number
should re-register a bar against today's frame and say which it is.
...
*"`tmdb_popularity` IS NULL on all 1,271,138 rows"* was a `--phase imdb` fact read

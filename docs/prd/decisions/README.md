...
| [0042](0042-the-watch-lane-resumes-from-a-startindex-checkpoint.md) | The watch lane resumes from a `StartIndex` checkpoint (not a `since` cursor), reused in place like `import_runs` | Proposed — fixes #41; makes the cursorless full-history watch walk completable rather than a restart loop, so `USHER_WORKER_ENABLED` can go back on |

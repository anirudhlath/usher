...
| **M10 — Hardening** ✅ | Observability, failure modes, backup/restore, docs, and the public release, `v0.1.0`. Plus the scheduler: a small set of named jobs with a period rather than a general cron, **no scheduler table** — each job reads *when was I last done* off the artefact it maintains — and `USHER_SCHEDULER_ENABLED=false`. `usher schedule --once` keeps the operator's-own-cron path supported |

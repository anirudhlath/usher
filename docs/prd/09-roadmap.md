...
| **M10 — Hardening** | Observability, failure modes, backup/restore, docs, public release. **Plus the freshness gap `CLAUDE.md` has carried since M6** — *nothing runs `usher similar --rebuild`*, issue #17, whose own "Done when" stopped at *"staleness is at least observable"* and deferred the automation to its own decision. ✅ **That decision is [ADR-0046](decisions/0046-the-scheduler-stores-nothing.md), and both its registrations have landed — J4's loop, J5's retention, J6's rebuild**: a small set of named jobs with a period rather than a general cron, **no scheduler table** — each job reads *when was I last done* off the artefact it maintains, so the fact has one copy and survives a restart — and `USHER_SCHEDULER_ENABLED=false`, because there is no mutual exclusion without a row and the job it would start is **3.34 hours** (M10's J7, 2026-09-08, measured on a clone at 93.4 ms/seed over 133,319 seeds). Two registrations: the neighbour rebuild — which J6 also taught to **resume** off its own artefact and to **refuse** a `title_embeddings` written by a model the deployment is not configured with — and the 90-day `search_queries` retention [10](10-telemetry-and-dashboards.md) prices and has nothing to run. `usher schedule --once` keeps the operator's-own-cron path supported rather than replacing it |
...
- **32 columns leak a raw driver exception across the port boundary, and 31 of
  ...
  **M9's boundary call 8 survives all of it, and ADR-0044 does not re-open it**:
  ...
  and does not model the COPY path. 🔴 **What ADR-0044 does retire is the
  ...
  **Four for four: the candidate is two changes, not one**, and the second one —
  ...
  than a module that never learned the mechanism. ADR-0044's first draft said
  ...
  **The one left is `jobs.attempts`, and translating it would have been the
  ...
  no port call can supply a value for it either. ADR-0044's *"What F9 did"*
  ...
  ADR-0044 flagged it as the one failure shape it took from the protocol and
...
- **`PortRateLimited.retry_after` reaches no consumer** — an M4 gap found by M8.
  ...
  **stub** — `FakeEmbyServer.rate_limit`, whose own docstring says it is the
  ...
  [ADR-0043](decisions/0043-the-outbound-limiter-is-per-source-and-spaces-requests.md)'s

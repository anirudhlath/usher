...
- **The coverage promise finally has denominators, and the one that matters is
  ...
  86,537 movies). **The rebuild this obliges has not been run** — 85.4 minutes
  at 130,647 titles, and `title_neighbors` is empty on every catalog on this
  host, which is why the acceptance that closes it must record that table's row

...
| **M10 — Hardening** | Observability, failure modes, backup/restore, docs, public release. **Plus the freshness gap `CLAUDE.md` has carried since M6** — *nothing runs `usher similar --rebuild`*, issue #17, whose own "Done when" stopped at *"staleness is at least observable"* and deferred the automation to its own decision. ⏳ **That decision is [ADR-0046](decisions/0046-the-scheduler-stores-nothing.md), taken here and not yet built**: a small set of named jobs with a period rather than a general cron, **no scheduler table** — each job reads *when was I last done* off the artefact it maintains, so the fact has one copy and survives a restart — and `USHER_SCHEDULER_ENABLED=false`, because there is no mutual exclusion without a row and the job it would start is **3.58 hours** (measured 2026-08-19 from `title_neighbors`' own timestamps). Two registrations: the neighbour rebuild, and the 90-day `search_queries` retention [10](10-telemetry-and-dashboards.md) prices and has nothing to run. `usher schedule --once` keeps the operator's-own-cron path supported rather than replacing it |

...
| `llm_calls` | ✅ The cost ledger ([10](10-telemetry-and-dashboards.md)): `(id, at, model, purpose, tokens_in, tokens_out, cost_usd, latency_ms, ok, error, generation_id)`. **No `user_id`, deliberately** — spend is attributed to an outcome by joining `curated_rows` on `generation_id`, which is what dashboard 5's "cost per curated row" *is*. `record()` is called on the failure path too, so `ok` is the discriminator and a ledger of successes alone understates spend by exactly the failures. `cost_usd` is **`NUMERIC(12, 8)`**, never a float: `$3/Mtok × 1,200 tokens` is exactly `0.0036` and at scale 4 a `$0.02/Mtok` call stores as `0.0000` — measured. `generation_id` is nullable (query expansion produces no rows) and carries no foreign key. ✅ **Two indexes since `m10c`**, and they are the two `m08a`'s docstring wrote down: `ix_llm_calls_at` for dashboard 5's spend-per-day panel and its cost-anomaly alert, and `ix_llm_calls_generation_id` — **partial, `WHERE generation_id IS NOT NULL`** — for its "cost per curated row" join, because query-expansion rows carry NULL and are exactly the rows that join never wants. ✅ **And they have a reader since M10**: `LLMCallRepository.list_since(since, *, until=None)` returns every *attempted* call in `[since, until)` ordered by `at` — `ix_llm_calls_at`'s own query, so `m08a`'s *"an index nothing reads is `ix_titles_popularity` again"* is discharged rather than only priced. The index ships one revision ahead of the read because M10 gets one migration and a reader task authoring its own DDL would be a second head. **The read's consumer is in `src/`, not Grafana** — [10](10-telemetry-and-dashboards.md)'s datasource table puts every spend panel on SQL in the dashboard JSON, so the caller that justifies the method is the cost-anomaly evaluation rather than dashboard 5. **No `WHERE ok`**, deliberately: the read is exactly as wide as the write, or it understates spend by exactly the failures. ✅ **The window is half-open** so a call landing on midnight is billed to one day rather than two, and `until=None` is unbounded rather than `now()`, because `at` is written by the caller and a row ahead of the server's clock is reachable. ✅ **Plan verified 2026-09-07** on PostgreSQL 17.10 (`pgvector/pgvector:pg17`) at head `m10c`: at 4,000 seeded rows a 24-hour window (24 rows, **0.6%**) plans as `Index Scan using ix_llm_calls_at`, cost 8.76, against 126.61 for the best plan reachable with that index suspended — **14.45×**. ⚠️ At 300 rows the same window still picks the index but by only **1.17×**, which is a tie-break rather than a property of the schema, and at 100 rows a `Seq Scan` wins and is correct. This table holds **0 rows / 16 kB** on the deployment (read-only re-measure 2026-09-07, unchanged since 2026-08-26) — ⚠️ **but that deployment's `alembic_version` is still `m10b`**, so the 16 kB is `llm_calls` carrying `pk_llm_calls` alone and is not a measurement of these two indexes at all. The table gains one row per generation per household per night, which is what bounds their write cost either way. ✅ **`cost_usd` verified exact end to end on 2026-08-07**: `0.00000000` against a local model, `0.01658700` with prices configured — exactly `Decimal((4359×3 + 234×15) / 1e6)` — and `SUM()` agrees to 8 decimal places. 🔴 **And it is the one table in this project rebuildable from *nothing*** — not from the catalog, not from `curated_rows` (replaced nightly), and not from any provider, since no OpenAI-compatible endpoint offers a per-key call history. It belongs in [08](08-operations.md)'s *precious* column, where M8 put it. Migration `m08a` |

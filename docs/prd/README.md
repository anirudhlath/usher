...
| [2026-08-21-issue-41-resumable-watch-lane.md](../plans/2026-08-21-issue-41-resumable-watch-lane.md) | Resumable watch lane (**neither a milestone nor an eval phase**): the watch-state walk checkpointed on its `StartIndex` so a transient failure costs one page rather than the whole ~5,688-page full-history walk, from [`docs/specs/2026-08-21-issue-41-resumable-watch-lane-design.md`](../specs/2026-08-21-issue-41-resumable-watch-lane-design.md) | 📋 **planned, nothing landed** — 6 tasks on `fix/41-resumable-watch-lane`, fixing [#41](https://github.com/anirudhlath/usher/issues/41): `sync_runs` gains a `position`, the newest *incomplete* `WATCH_STATE` run is reclaimed rather than superseded, and `SourceAdapter.watch_state` grows a `start_index`. Why a `StartIndex` checkpoint rather than the `since`-timestamp cursor the issue's wording suggests is [ADR-0042](decisions/0042-the-watch-lane-resumes-from-a-startindex-checkpoint.md) |

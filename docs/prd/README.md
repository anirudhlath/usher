...
| [2026-08-13-m10-hardening.md](../plans/2026-08-13-m10-hardening.md) | M10 — Hardening ([08](08-operations.md)'s failure, backup and secrets sections, all of [10](10-telemetry-and-dashboards.md)'s dashboards and alerts, and the public release) | 🚧 **in progress** (Phase 1 started) — 67 tasks in four gated phases (Foundation → Safety → Operability → Release) on `milestone/m10-hardening`, cut from `f2cf359`; its design spec sits beside it in [`docs/specs/`](../specs/). **Phase 1 (Safety) is underway** — S1 has run (below); the rest of the milestone is not yet built. **Its parallel drafting pass refuted fifteen of that spec's own claims before a line was implemented** — see the plan's *Corrections this plan carries*. Two changed a justification rather than a detail: the neighbour rebuild is **3.33 h, not 21.6** (594.7 ms/seed was `m09e`'s and `m09f` repaired it to 91.7) — ⚠️ **and 3.33 h is itself 2026-08-13's; the walk this deployment last completed is 12,884 s over 132,442 seeds, 97.3 ms/seed, 3.58 h, read off `title_neighbors.computed_at` on 2026-09-07 by M10's J6**, and the **"~1–5 s/request" source latency the whole safety cluster was scoped against had never been measured** — it entered in `0c823e0`, the first PRD commit, two days before an Emby adapter existed, was cited 22× and called *measured* 11×. **S1 measured it on 2026-08-15** and it was never about a request: `verify` 0.1253 s, `get_item` 0.1495 s, a 200-item `list` page **5.0954 s median / 6.0369 s mean** — bimodal, ~34× apart, and the page is *dearer* than the string, not cheaper ([01](01-architecture.md), `.claude/rules/emby-push-and-ingest.md`) |
| [2026-08-18-e1-eval-skeleton-and-suggest.md](../plans/2026-08-18-e1-eval-skeleton-and-suggest.md) | E1 — quality evals, phase 1 of 4 (**not a milestone**): the `usher.eval` skeleton and the suggest surface, from [`docs/specs/2026-08-18-usher-quality-evals-design.md`](../specs/2026-08-18-usher-quality-evals-design.md) | ✅ **merged 2026-08-20 ([PR #45](https://github.com/anirudhlath/usher/pull/45))** — Tasks 1–13 and 15 landed: the `eval` package and its extra, the eleventh **and twelfth** import contracts, hashed pre-registered bars, the two ledger sinks, and Task 15's sweep (19 plants, 16 killed, 3 predicted). ⏳ Task 14 Steps 3–4 (filling the three `fuzzy recall_at_5` bars) stay open on [#39](https://github.com/anirudhlath/usher/issues/39) — filling them from the unrepaired system would pin a bar its fix fails. This cell read "Task 1 landed" until 2026-09-02. **E2–E4 have no plan files**; they are designed as phases in the spec, and are *not* the `E2`/`E3`/`E4` task ids in [09](09-roadmap.md), which are M9's |
| [2026-08-19-rating-provenance-split.md](../plans/2026-08-19-rating-provenance-split.md) | Rating provenance (**neither a milestone nor an eval phase**): the three `titles` columns written by two sources each, from [`docs/specs/2026-08-19-rating-provenance-split-design.md`](../specs/2026-08-19-rating-provenance-split-design.md) | ✅ **all 7 tasks landed 2026-08-19** (this cell opened "🔨 in progress — Task 1 landed" until 2026-09-02, contradicting its own ✅ two paragraphs down). Task 1 was migration `m10a`, which renames `community_rating` to **`tmdb_vote_average`** (not a prefix — the only one of the three that is a rename), `vote_count` to `tmdb_vote_count` and `popularity` to `tmdb_popularity`, adds `imdb_average_rating`/`imdb_num_votes` beside them, and renames `field_provenance`'s three JSONB keys to match — the only statement in the revision that touches a row, and a rename rather than an inference (all 132,415 rows carrying provenance carry all three keys and every value is `tmdb`). No rating **value** is migrated: those are re-imported from `title.ratings.tsv.gz`. Its follow-up also restored `GET /events`' `title.updated` payload to wire field names, which the rename had moved to domain attribute names — the one place a field name travels as data rather than as a key. ✅ **All 7 tasks landed 2026-08-19** — the IMDb writer redirected onto the `imdb_*` pair, `usher bootstrap --phase ratings` added (an alias, not a step, and it refuses an empty catalog), E1's sampling frame re-anchored on `imdb_num_votes`, and the rebuild run against the live 1.27M-title catalog: ADR-0002's frame is restored to **48,639 against 48,549 (+0.19%)** from **8,523** under the contaminated column. The decontamination was the one component that did **not** go as designed: its pre-registered exact-match rule misses 57,701 of 407,860 rows, because the column held 2026-08-11 IMDb values and the re-import brought 2026-08-19 ones, so P3 would have failed. It was reported as an open decision rather than widened unilaterally; the operator then authorised the `enrichment_state` rule, which was applied the same day — **P3 passed at exactly 40,695**, `tmdb_vote_count` fell from 540,275 to the 132,415 enriched rows, and no non-enriched row carries one. [ADR-0040](decisions/0040-rating-columns-name-their-source.md) preserves the sequence. Rollback table `titles_rating_backup_20260819` is intact and must not be dropped without an operator's say-so |
| [2026-08-21-issue-41-resumable-watch-lane.md](../plans/2026-08-21-issue-41-resumable-watch-lane.md) | Resumable watch lane (**neither a milestone nor an eval phase**): the watch-state walk checkpointed on its `StartIndex` so a transient failure costs one page rather than the whole ~5,688-page full-history walk, from [`docs/specs/2026-08-21-issue-41-resumable-watch-lane-design.md`](../specs/2026-08-21-issue-41-resumable-watch-lane-design.md) | ✅ **all 6 tasks landed 2026-08-25/26; merged 2026-08-26 as [PR #68](https://github.com/anirudhlath/usher/pull/68)** — `fix/41-resumable-watch-lane`, for [#41](https://github.com/anirudhlath/usher/issues/41), **which is still open**: the PR title says "Fix #41" but GitHub only auto-closes on a merge to the *default* branch's linked issue, and it did not fire here, so the issue needs closing by hand (or is being held for the completion walk below): `sync_runs` gains a `position`, the newest *incomplete* `WATCH_STATE` run is reclaimed rather than superseded, and `SourceAdapter.watch_state` grows a `start_index`. Why a `StartIndex` checkpoint rather than the `since`-timestamp cursor the issue's wording suggests is [ADR-0042](decisions/0042-the-watch-lane-resumes-from-a-startindex-checkpoint.md). **Review added two behaviours the plan did not have**: a non-destructive `save` (`position` only advances, `completed` is absorbing) after the spec's *"no true concurrency exists to race"* was measured false, and merges stamped with the attempt's instant rather than the reclaimed run's. ⏳ **The walk itself has still never been run to completion** — that is the operator's step after this merges (nothing schedules one), and the branch review measured convergence at 3–10 attempts |
| [2026-09-12-comment-convention-and-polish.md](../plans/2026-09-12-comment-convention-and-polish.md) | M10 polish (**not a milestone of its own**): the comment convention, the `/simplify` findings, and the prose cut across code, `decisions/` and the PRD | 🚧 in progress — stage 1 of 4 |
| [2026-09-12-stage-1a-shared-infrastructure.md](../plans/2026-09-12-stage-1a-shared-infrastructure.md) | M10 polish, stage 1A: the reader slot, the narrow scope, the non-finite refusal | 🚧 in progress |

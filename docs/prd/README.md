...
| [2026-08-21-issue-41-resumable-watch-lane.md](../plans/2026-08-21-issue-41-resumable-watch-lane.md) | Resumable watch lane (**neither a milestone nor an eval phase**): the watch-state walk checkpointed on its `StartIndex` so a transient failure costs one page rather than the whole ~5,688-page full-history walk, from [`docs/specs/2026-08-21-issue-41-resumable-watch-lane-design.md`](../specs/2026-08-21-issue-41-resumable-watch-lane-design.md) | 📋 **planned, nothing landed** — 6 tasks on `fix/41-resumable-watch-lane`, fixing [#41](https://github.com/anirudhlath/usher/issues/41). `WatchStateSyncService` takes its cursor from the newest **completed** watch-state run, so until one completes the walk is the whole library and any transient failure over it restarts the walk from zero — a loop that has never once completed on the measured deployment, and whose workaround in force is `USHER_WORKER_ENABLED=false`, i.e. every queued job stopped to stop one lane. The fix is `import_runs`' "checkpoint updated in place" applied to one sync lane: `sync_runs` gains a `position`, the newest *incomplete* `WATCH_STATE` run is reclaimed rather than superseded, and `SourceAdapter.watch_state` grows a `start_index`. **Not** a `since`-timestamp cursor, which is what the issue's own wording suggests and which the sort order makes unsound — [ADR-0042](decisions/0042-the-watch-lane-resumes-from-a-startindex-checkpoint.md) records why |

...
| Worker in its own process (`USHER_WORKER_ENABLED=false` on the server, `usher work` beside it) | ✅ M9: every SSE frame a *job* raises reaches a `NullEventPublisher` and no client is told — `title.updated` since M5, and `bootstrap.progress` since M9's E7 put `JobKind.BOOTSTRAP` on the queue. The bus is in-process and the `LISTEN/NOTIFY` implementation `ports/events.py` names has no owner. **Nothing durable is lost**: the catalog, `import_runs` and `sync_runs` are written by the worker either way, so `GET /admin/bootstrap/status` and `GET /admin/sources/{id}/status` report the same thing in both topologies and a client that heard nothing can still see where a run got to. The cost is latency to a *screen*, not correctness. ✅ **M10 F10: the two topologies now also agree about what one crashed pass costs, and until F10 they did not.** The lane caught `Exception`, logged and looped; `usher work` had no per-pass `except` at all, and `JobWorker._pass` re-raises the first task failure after every task has settled — so one job's bug ended the daemon while the identical job on the server cost one pass. The daemon has the lane's arm now, and **both roots record the failure with `logger.exception` rather than with `str(exc)`**, which is issue #8's other half: a message loses the frames, and a lane that survives leaves no dead process to notice either. ⚠️ **`usher work --once` is deliberately outside the arm** — a cron entry reads the exit code, and a guard there would answer a crashed pass with `0`. |
...
catalog, the most recent completed rebuild took **3.34 hours over 127,420
seeds** — M10's J7,
...
| Image cache | **Bounded per image and unbounded over time**, and the two halves are why this row is not a single number. `services/images.py` states the first: *"There is no resize, no format decision and no eviction — … the ladder bounds the cache at four entries an image by construction"*, the ladder being `IMAGE_LADDER = (154, 342, 780, 1280)` in `ports/images.py`. The second is that **nothing evicts and the catalog grows**, so the size is `images browsed × up to four rungs × their bytes` — **the growth driver is browse coverage, not catalog size**: an image nobody has asked for costs nothing, and one somebody has asked for at four widths is there for ever. ⚠️ *"capped by a configurable LRU ceiling"* was wrong from the day it was written: **there is no eviction method to configure** — `DiskImageBlobStore` has `get`, `put`, `_forget_other_media_types` and `_path` — and none of the four real settings is a ceiling. They are `image_cache_dir` (where), `image_max_bytes` (a **per-image** 5 MiB refusal, never a cache cap), `image_fetch_timeout_seconds` (how long one fetch may take) and `image_cdn_base_url` (whose CDN). Measured on this deployment 2026-09-11: **146,056,327 B over 1,116 files for 828 distinct images** — 609 at one rung, 155 at two, 59 at three, 5 at four and **none at five**, which is the ladder's bound showing up in the data; mean **127.8 KiB** a stored rung. 828 images against a 1,276,268-title catalog is **0.06% browse coverage**, so extrapolating today's mix (1.35 rungs an image) to one image per title gives **~225 GB**, and all four rungs **~668 GB** — an extrapolation, labelled as one, and the reason this directory is half of PRD 10's *Disk projection* alert. Nothing reclaims it. |
